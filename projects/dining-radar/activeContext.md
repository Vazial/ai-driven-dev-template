...
**会の画面群の描き直しは PR #192（`integrate/gathering-redesign`→`main`）で CI 全緑、マージ待ち。**
#192 は #190（実機フィードバックの控え）と #191（会の画面に描画不変量を掛ける）の内容をすべて含むため、
2つは閉じた（同じ ADR 番号を使っていて、残すと ADR 採番の検査が落ちる）。

**その上に、残り1件の店のトグルを直す slice を積んでいる**（ブランチ `integrate/last-shortlisted-shop-toggle`、
base は #192）。会に入れた店が残り1件だと、トグルを押しても何も起きなかった（`minItems: 1`、adr/0041 P2）。
...
1. **PR #192（会の画面群の描き直し）をマージしてもらい、その後に残り1件のトグルの PR の base を `main` へ付け替える**（上の「いま進行中のもの」）。
