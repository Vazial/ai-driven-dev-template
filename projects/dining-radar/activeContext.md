...
**束E（店を絞る画面）の3件のうち2件をADR-0064で実装した**
（ブランチ `feat/dining-radar-bundle-e-candidates`、PR待ち）。人間がdesignerの板
（`E:/AWS/dsg-out/party2/e1/E1-a-*`・`E2-a-*`）を見て裁定した2点——(1)「もう一度探す」を
「別の候補を出す」に改称し、スマホも丸いアイコンだけの形をやめて文字を付ける（動作・置き場所は
無変更）、(2) カード末尾だった「定休日」の行を、席数・禁煙・夜予算と同じ枠（`cardDataAttributes.
detailGroup`）の1項目へ移す（値・無いときの表記は無変更）。候補画面の観測面は1.12.0から1.13.0へ。
testId・purposeはいずれも既存を再利用し新設なし。会モードでも同じ見た目であることをスクリーンショット
で確認済み。手元でL1（単体49＋既存分すべて緑・カバレッジ98%）・L2・L3（`test_structure.py`・
`manage.py check`双方）・L5（`test_render_invariants.py`全件）を実行して緑。欠陥注入4件
（ラベル文言差し戻し・スマホの丸ボタン差し戻し・定休日フィールドの旧位置差し戻し・facts枠を
1列に戻す）はいずれも狙った検査だけが落ち、戻して緑に復帰することを確認した。**3件目（徒歩◯分の
実態乖離）はADR-0064の対象外のまま**（下記の人間の判断待ち）。L4は tester がこのブランチで並行して
扱っている。
