...
**会の画面群の描き直しを統合し、全層緑にした。PR はまだ出していない**（ブランチ `integrate/gathering-redesign`）。
このブランチは PR #190（実機フィードバックの控え）と #191（会の画面に描画不変量を掛ける）の内容を
すべて含むので、**マージしたら #190・#191 は閉じてよい。**
...
1. **会の画面群の描き直しを PR にする**（上の「いま進行中のもの」）。検証は L0〜L5 すべて緑、PR を出してマージ待ち。
