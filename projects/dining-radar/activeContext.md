...
- **検証**（統合後のブランチで orchestrator が実行）: L0 govlint 緑、L1 単体 733件・分岐カバレッジ 98%、
  L2 構造 13件、L3 check ×2 緑、**L4 受け入れ 86件 OK**。L5 描画不変量は、統合後の全体実行で
  **待ち方の競合1件**（会の画面の描画を待たずに操作要素を数えていた）が出たため、developer が再現してから直している最中。
...
1. **会の画面群の描き直しを PR にする**（上の「いま進行中のもの」）。L5 の待ち方の競合を直したら出す。
