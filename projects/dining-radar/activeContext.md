...
**スマホの大きさの違いへの追従（ADR-0065、ブランチ `feat/dining-radar-responsive-phone`）。**
人間の依頼「スマホの画面が多少解像度が変わっても調整されるように」。見た目の裁定（ADR-0059〜0064）は変えず、
画面の高さの基準を見えている高さ（`dvh`）に、見出しの高さを固定値で引く作り（`calc(100vh - Nrem)`）を
伸び縮みする並べ方に、シートの高さを板の比率＋上限下限に直した。「リンクを発行」の小窓が 320〜1024 幅で
右にはみ出していた実害（幅の下限 20rem 固定が原因）も直した。

- 描画不変量はスマホ5サイズ・横向き・タブレット／PC で、横スクロール（小窓を開いた状態を含む）・固定の帯の
  収まりと覆い・地図の見えている高さ・シートの中身（店の行、確定後の日時と店の行と開閉行）が見えることを測る。
  320×568 と横向きは、理由を書いて一部の条件を外している。検査に使う店は乱数の種で固定し、別に合成データで
  一番長い店名の店でも崩れないことを確かめる。
- orchestrator が実装の画像を板と並べて見る工程で、検査が緑のまま起きていた後退を2回止めた（横向きのために
  シートを全サイズで削って店の行や確定後の中身が隠れた／360×740 を検査の対象外にしていた）。「人間が見る前に
  細かい崩れを捕まえる」工程の組み立て（人間の崩れの見本を集め、機械で捕まえる範囲と画像で見る範囲を決める）は
  このあと進める。
