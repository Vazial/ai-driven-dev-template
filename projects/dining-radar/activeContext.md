...
### 第5束「確定」の裁定（2026-09-12、チャット選択肢UI）——**これで5束すべて決着**

デザイナーの板: `E:/AWS/dsg-out/party/b5-final/`。送り付きページ: `scratchpad/pager/b5.html`。
1枚目に**5束ぶんの総括**（契約の不足20件・覆す必要のある既存決定10件・つながりの全体図の最終形）。

- **確定後の参加者の画面は5つだけ見せる**（この順）: いつ／どの店／**どこにあるか（地図1枚、
  店のピンと勤務地）**／徒歩の目安／店のページへの線。
  **2点を結ぶ線は引かない**——道順の主張になる。経路検索に問い合わせない製品は、その情報を
  持っていない。持っていないものを絵で言わない。徒歩の輪も描かない（比べる相手がいない）。
- **「あなたの日程への回答」1行は消す。**`adr/0050` 決定3（確定後は1行だけ）の一段先へ進む。
  決まったあとに自分の過去の回答を読んでも、行動が1つも変わらない。
- **確定後も参加者は店ごとの票を見られるままにする。**——**デザイナーの板は見せない側で
  描いているので、板と裁定が食い違う。**実装は裁定に従うこと。
  契約は同じ節の中で見られるとも見られないとも書いており（`decision.requirement` と
  `replacesQuestionSurfaces`）、**この節は同種の矛盾を過去に一度直した記録が残っている場所**である。
- **消さなかった会は、手で消すまで残す。**`product-brief.md` §8 の未決（保持期間・削除方針）が
  これで決着する。人間の「永久に残っちゃう？」への答えは「消すまでは残る」。
- **確定の確認に「変わること」の表を3行置く**（回答リンク／参加者の画面／日と店 が前→後でどう
  変わるか）。読ませる文ではなく矢印の表なので、説明文を置かないという決めごとに触れない。
- 確認の一段は**削除の確認と同じ形**にし、違うのは色と中身の性格だけ（確定＝緑「変わることの表」／
  削除＝`#8A3B2C`「失うものの一覧」）。確定ボタンは店名を言わない（すぐ上の帯にある）。
- 確定後の票の記録からは**票の1位の緑の印を消し**、決まった店の濃墨だけを残す。

**契約に足りない要素（第5束で新たに判明した分）**:
- **確定に確認の一段が無い。**提案は既存の `gathering-finalize-submit` を「確認の面を開く／
  公開の操作は呼ばない」へ変え、`-confirm-dialog`/`-confirm`/`-cancel` を足す形。
- **参加者の確定後画面に店の場所を見せる面が無い。**ただし**API の変更は要らない**——
  `LiveProjectedShop` が `name`/`location`/`walkingTimeMinutes`/`providerPageUrl` を持ち、
  `ParticipantView.searchOrigin` は確定後も非 null。足りないのはブラウザ契約の観測面だけ。
- 確定後の `data-current-leader` の扱い／削除後の行き先／「発行はおわり」の観測面／
  `data-your-schedule-response` を残すか消すか。

**次にやること**: architect が **(1) 5束ぶんの裁定を ADR に落とし、`product-brief.md` を改訂する**、
...
**保留（次のラウンドへ）**:
- **会に入れた店が残り1件になると、その店のトグルを押しても何も起きない**（2026-09-13 統合で判明）。
  `SetShortlistedShopsRequest.shopIds` の `minItems: 1`（P2、2026-09-03 の人間裁定）により、最後の1件を
  外す要求はサーバが 400 で拒否し、画面は契約どおり状態を変えない。**契約どおりだが、利用者からは
  押しても反応しないトグルに見える**——人間が以前「機能してないようにみえる」と指摘したのと同じ形である。
  上限（5件）には `data-shortlist-limit-reached` と帯の表示で理由を示すようにしたが、**下限（1件）には
  理由を示す面も、押せないことを示す面も無い。**直すなら (a) 最後の1件のトグルを押せない状態にして理由を示す、
  (b) 最後の1件も外せるようにして店を選び直す状態へ戻す（P2 の裁定を覆す）、のどちらかで、**人間の判断が要る。**
