...
**第3回の実機フィードバック（2026-09-22、人間が画像8件つきで提示）から、板と実装の突き合わせを始めた。**
ブランチ `feat/dining-radar-board-conformance`（PR 準備中）。

### 分かったこと（根が同じ）
人間が板で裁定した形（例: PC の ≡ は「右上・右から出る」）が、実装では左上に置かれ、L0〜L5 すべて緑のまま
main に入っていた。原因は3つ——(1) ADR-0059 が位置を「見た目の実装選択」として契約の必須条件から外した、
(2) 実装が板から外れた、(3) orchestrator が実装の画像を板と並べて確かめていなかった（束A〜Cは検査結果だけで
PRにしていた。画像で見る工程は束Dから）。記録の消失でも文脈の圧縮でもない。

### 作った仕組み（このブランチ）
- `tests/ui_invariants/test_layout_sanity.py`: 画面を問わない崩れの検査8種（重なり／文字の切れ／意図しない
  折り返し／はみ出し／窮屈な余白／開く向き／押しても何も起きない部品／文字と背景の見分けやすさ 4.5・3.0）。
  いまの実装で落ちる9件は理由つきで xfail に置いてある（直したら外す）。
- `tests/ui_invariants/test_zz_capture_states.py`: `DINING_RADAR_CAPTURE_DIR` を付けたときだけ動き、
  9画面×34状態×6サイズ＝204組の画像と部品の位置を書き出す（通常は skip）。
- orchestrator の道具（リポジトリ外）: `pager/board_compare.py`。板の画像と実装の画像を並べたページを作り、
  板の無い状態を赤く印す。対応表は `scratchpad/compare-manifest.json`。
- ADR-0066（この8種を ADR-0020 決定4 の一覧に足す／板で決めた位置・並び・開く向きを以後は契約の Must に／
  撮影と突き合わせを UI の束の PR 前の必須手順に）と、候補画面の観測面 1.14.0（≡ とチップの位置、メニューの
  出る向き）。

### 突き合わせの結果（`scratchpad/audit-2026-09-22.md` に全文）
合意から外れていたもの13件、決め直しが要るもの4件、板に関係ない崩れ1件。重いものは次の3つ。
- 日程を聞いている幹事画面が、板（人×日の表・有力・決めるボタン・候補日の札）ではなく、候補日の大きな
  カードが縦に並ぶ形のまま。第2便の指摘がそのまま残っている。
- PC の ≡ の位置と開く向き（人間の指摘1・4）。スマホのアカウントのシートも板と違う。
- 参加者の日程回答・確定後の回答タブ・リンク発行の小窓・会をつくるの確認の小窓が、それぞれ板と違う。

### 人間の判断待ち（板は `E:/AWS/dsg-out/party2/f1/`、描画済みは `scratchpad/f1/`）
1. ランチ会への行き先が2つある件（案1 いまの決まりどおり／案2 チップをやめる／案3 ≡ はアカウント専用）。
   **選択肢は提示済みで未回答。**
2. ⓘ（消す／押せる形にする）。3. 店0件の画面（いまの契約どおり／地図いっぱいに揃える）。
4. 日程タブの中身（候補日を畳む／開いたまま）。

### 次にやること
1. この仕組みのPRを通す（L4 が日付依存で落ちていた件は、候補画面の下準備の日付を1日ずつ進める形に直した）。
2. 上の4件の裁定をもらう。
3. 「合意どおりに戻すだけ」の13件を画面ごとに束ねて直す。直すたびに板と画像で確かめてからPRにする。
4. 徒歩時間の計算（実測の対を人間からもらう）と、束Eの残り。
