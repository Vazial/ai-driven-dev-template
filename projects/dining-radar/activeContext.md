...
実装し直した（決定2）。浮かせた一覧・シートは、板（`party2/d7/S4-SpSelect.dc.html`・
`party2/d4r/G2-SpFinal.dc.html`）自身の比率に戻している——店を選び中のシートは
`min(64%, calc(100% - 4.25rem))`（板の64%を尊重しつつ、面の上端より上に出ないことを式で保証）、
確定後のシートは面の下端に貼り付け（`bottom: 0`）・高さは中身依存で上限のみ画面割合
（`max-height: 85%`、収まらなければ内部スクロール）。実測で見つかった実害——「回答リンクを発行」の
小窓（`.gth-issue .gth-confirm-dialog`）が固定幅下限（`width: max(20rem, 100%)`）のため320〜1024幅で
右にはみ出す——は、開いている親要素の幅を超えない値へ直した。360×740（一般的なAndroid幅）で店の
一覧の見出し行が折り返し店の行が隠れていた件は、スマホ幅の外枠・見出し余白を詰め、「票が多い順」を
ボタン2つと別行の小さな見出しに分けて直した。見た目（ADR-0059〜0064の板の裁定）は変えていない
（決定7）——比率・貼り付け位置は復元であり変更ではない。
...
こと、(i)地図主体の画面で地図の見えている高さが一定以上（120px・20%の複合下限）であること。測定する
ビューポート集合をスマホ5機種＋横向き＋タブレット/PC代表3幅（`RESPONSIVE_MATRIX_VIEWPORTS`）へ広げた
（決定4）。(i)の緩和は横向きと320×568のみに限定した——390×844・360×740は板の比率のまま本来の下限を
安定して満たす（下記の乱数固定により間欠性が解消したことを確認済み）。店を選び中・確定後それぞれの
画面について、シート開閉状態での店の行・確定した店の行・回答/回答リンクの開閉行2本が縦持ち5サイズ中
320×568を除く4サイズで丸ごと見えることを検査する内容可視性の不変量も追加した。**検査対象の店は
`RESPONSIVE_CHECK_RANDOM_SEED`で決定的に固定している**（合成データの重み付き抽選を種固定なしで
使うと、どの店が選ばれるかが実行ごとに変わり可視性検査が間欠的に揺れたため）。加えて、合成データ中で
最長の店名を持つ店（15回の候補取得を和集合して発見）が横にはみ出さず2行に収まることを確かめる検査を
1本新設し、実際に見つかった崩れ（3〜5行への折返し）を`.gth-shop-name`への`-webkit-line-clamp: 2`で
直した。

確定後の地図の「決まった店」の名札（Leafletの常時表示ツールチップ、`.gth-map-label`）が、長い店名の
とき地図の右端に達し拡大縮小ボタンに重なっていた（実機スクショで発覚）。名札自体に`max-width: 6.5rem`
＋1行末尾「…」（スマホ・PC共通）を付け、`initializeOrganizerDecisionMap`（gathering.js）の
`fitBounds`の名札側余白（`MAP_LABEL_FAR_SIDE_CLEARANCE_PX`）をデスクトップの160pxへスマホも揃えて
直した。地図面への収まり・拡大縮小ボタンとの非重なりを検査する不変量を1本新設（縦持ち5サイズ＋
タブレット幅、最長店名の店で）。**既知の別件（このラウンドでは対象外）**: 320×568だけは、検索基点と
決まった店の合成データ上の座標が近く、地図が最も狭いこの1幅でのみ両者の名札同士が重なる
（`MAP_LABEL_FAR_SIDE_CLEARANCE_PX`の値に関わらず起きることを実測で確認済み——この回の変更が
起こしたものではない、既存の別問題）。他の必須幅（360×740〜430×932・横向き・タブレット/PC）では
起きない。

(g)(h)(i)・内容可視性・最長店名・地図名札の各検査は、それぞれ欠陥注入→落ちる→戻す→緑、の一往復を
確認済み。手元でL1（単体825・ruff・カバレッジ98%）・L2/L3（`manage.py check`×2）・L4（受け入れ113、
全緑）・L5（描画不変量58件・76 subTests、全緑）を実行し、responsive系の検査は個別に反復して
安定（非間欠）を確認した。ADR-0065はこの実装時点で`提案中`（人間未承認）——中身は上の決定と一致。
