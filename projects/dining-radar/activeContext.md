...
**残り1件の店のトグルを直す slice も PR #193 で `main` にマージ済み（2026-09-13）。**会に入れた店が残り1件だと、トグルを押しても何も起きなかった（`minItems: 1`、adr/0041 P2）。
...
- **契約の版**: 会の API v0.15.0／会の観測面 0.19.0／候補画面の観測面 1.11.0／
  TDR-GTH-01〜56・TDR-CS-00〜22（07 廃止）／検査支援 v1.5.10。
...
- **契約の書き直し方**: architect は全文書き出ししか持たず、大きい契約は1回の出力上限を超える（FR-023）。
  ...
  道具は `E:/AWS/Claude_Workspace/orchestrator-tools/` の `apply_spec.py`・`check_contract.py`（リポジトリ外）。
