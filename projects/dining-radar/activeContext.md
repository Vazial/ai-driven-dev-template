...
**(3) は決着した（人間裁定 2026-08-23）。ただし案2でも案3でもない第3の形である。**
...
- **最初は店舗のリストでよい。タップしたら地図を開く**（案2に近いが、地図は「1行のバーに畳む」のでは
  なく、リストから遷移して開く）
- **地図は現在選択されている店舗を中心に表示する。その下にその店舗の情報を出す。それ以外の店舗は
  出さなくてよい**——つまり地図画面に横スワイプのカードデッキは要らない（「画面幅は不要」）
- **別の店舗を見たいときは、地図上のピンをタップして切り替える**

すなわち**地図は候補一覧の一部ではなく、選択中の1件の詳細ビューである**。案2が保っていた「地図の上で
候補を横に比べる」も、案3の「地図を主役にしてカードを重ねる」も、どちらも採らない。比べるのはリスト側の
仕事であり、地図側は1件を確かめる仕事だと分けたことになる。

**この形はまだ描かれていない。** designer に渡して描き起こす必要がある。描き起こす際に確認が要る点:
比較で判明した「案2を選ぶと契約の追補が要る」（下記）が、この第3の形にも当てはまるかどうか——初期表示が
リストで地図が別画面になるため、`candidate-search-browser-interface.yaml` が初期表示に `candidate-map` の
存在を要求している条文との関係は、案2のときと同じ論点になる可能性が高い。
