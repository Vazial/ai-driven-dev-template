...
10. **次の大きな一手（2026-08-29 時点。人間の意向）: 会そのものを扱う画面群（幹事の画面）へ移る。**
   ...
   **人間の決定が下りた（2026-08-29、チャットの選択肢UIで4問を確定）**:
   ...
   これを受けて architect がブリーフ改訂＋`adr/0034` のドラフトを起草中（ブランチ
   `docs/dining-radar-party-scope-brief`）。承認は `meta/adr/0035` 方式(i)——本PRのマージが承認行為。
   キャンバス未取り込み5枚の抽出テキストは orchestrator が機械抽出して architect へ渡した（キャンバス
   注記に探索時点の判断——締切なし・未回答でも進行・分母は回答者数・「決定は幹事、票は材料」——と、
   唯一の明記された未決「確定後に届いた回答の扱い」が残っている）。
