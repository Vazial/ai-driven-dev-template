...
>
> **2026-09-01追記（ADR-0038が下記の未決事項2・3の状態を更新）**: 本ADR自体の決定・検討した代替案・
> 帰結の本文はP-06に従い変更していない。未決事項2（候補日追加フォームの画面）は2026-09-01の人間裁定
> （案A「その場に開くインラインフォーム」）で決着し、未決事項3（リンク管理面の画面が正規パイプライン
> 未通過）はdesignerによる`Organizer.dc.html`の再描き直しで実態として解消した——いずれも下記の
> 未決事項節に決着として記録する。詳細は`ADR-0038`を参照する。
...
1. **`ADR-0035`から持ち越し**: 会データの保持期間・削除方針、署名付きリンクの具体的な有効期限
   日数・レート制限閾値（いずれも根拠の薄い暫定値）は引き続き未決である。`test-support-api.yaml`の
   TDR-GTH向け拡張は`ADR-0037`が着手し、決定7が追加したリンク管理3操作の分も含めて拡張済みである
   （2026-09-01時点で解消済み）。
2. ~~**候補日追加フォームの画面**: `gathering-add-candidate-date-open`が開く先の入力フォーム自体は
   ...
   **決着（2026-09-01、`ADR-0038`。designerがAddDate.dc.htmlで2案を描き、人間が案A「その場に開く
   インラインフォーム」を選んだ）**: `gathering-add-candidate-date-form`・
   `gathering-add-candidate-date-input`・`gathering-add-candidate-date-submit`／`-cancel`として
   `gathering-scheduling-browser-interface.yaml`へ追記済みである。詳細は`ADR-0038`を参照する。
3. ~~**リンク管理面の画面自体はまだ正規パイプラインを通っていない**: 本ADRの契約はAPI・観測面の
   ...
   **決着（designerが正規パイプラインで`Organizer.dc.html`を再度描き直したことを確認、`ADR-0038`が
   記録）**: 再描画版は発行済みリンクの一覧表・3局面（下書きの廃止を反映）への縮小した局面ピルバー・
   「押すたびに1本ずつ発行」の注記を含み、本ADR決定7の制約（トークン非露出、失効は未回答のみ）を
   満たした状態で描かれていることを確認した。新たな設計判断は伴わない——事実の記録のみである。
