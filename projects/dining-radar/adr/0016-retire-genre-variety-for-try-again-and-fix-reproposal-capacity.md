...
**（2026-08-10 追記。`adr/0023`、提案中・未承認のドラフト。friction-log FR-015）** 本ADRの記録には
内部矛盾がある。「検討した代替案」節は「(b) 毎回違う店が出るよう決定性を再検討する」を「却下（人間
裁定）」と記録しているが、その却下理由として挙げられているのは`product-brief.md`・`ADR-0004`・
`ADR-0005`・`ADR-0008`・API仕様の引用だけであり、**人間自身がこの提案を退けた理由はどこにも記録
されていない**。一方、本ADRの「文脈2. 人間の指摘と判断」がそのまま引用する人間自身の言葉はこうである。

> そもそもシャッフルしてればジャンルもばらけるはずだから、選び方を廃止してシャッフル機能にするほうが
> いいかもしれないね／前回出した店舗は出さない（優先順位を下げる）の方針でどうかな

**この発言で人間はシャッフル（無作為性）の採用を自ら提案していた。** 実際に実装されたのは発言の後半
（優先順位を下げる、決定2・3が実装した既存の再表示降格）だけであり、前半（シャッフル）は検討・
再確認されないまま「却下（人間裁定）」という体裁で記録された。この追記は、`architect`（`adr/0023`
起草時）が当初この却下の記録をそのまま引用し、人間の再承認を求める根拠として「人間が過去にこの提案を
明示的に却下した」と誤って記載したことをorchestratorが検知し、friction-log FR-015
（cause_key: `adr-records-rejection-contradicting-human-quoted-words`）として記録した経緯を受けた
ものである。**本ADRの決定本文（決定1〜8）は書き換えない**——実装済みの内容（`GENRE_VARIETY`廃止・
「もう一度探す」の導入・決定的ルールの維持）自体は人間裁定として成立しており、有効である。変わるのは
「シャッフルの提案を人間自身が却下した」という、この却下の記録に付随していた前提だけである。詳細と
判定根拠は`adr/0023`を参照する。
