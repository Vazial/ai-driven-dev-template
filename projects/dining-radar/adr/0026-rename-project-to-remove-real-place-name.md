...
着手時の実測（tracked のみ）は次のとおりで、切り出し時のメモの見積もり（170ファイル/629箇所）とは
大きく違った。差は本体の作業コピーに存在するビルド生成物（`node_modules` 等）を数えていたことによる。

- `toyama-dining-radar`（ケバブ）127箇所 ＋ `Toyama Dining Radar`（表示名）13箇所 ＝ **56ファイル/140箇所**
...
### 採番について（0025ではなく0026を名乗る理由）

本ADRの起草時点で `adr/0025` は**未push・未マージのローカルブランチ
`docs/tdr-cs-origin-and-walking-time` が既に使用中**である
（`adr/0025-disclose-search-origin-and-walking-time-to-the-authenticated-screen.md`。
検索基点と徒歩時間を認証済み画面へ出す判断で、TDR-CSの契約4本と `product-brief.md` の改訂を伴う）。
`meta/adr/0026` が同じ状況で0025を避けて0026を名乗った先例に倣い、本ADRは0026を名乗る。

**このブランチは `gh pr list` に現れない**——PRが開かれておらず、リモートにも push されていない。
`meta/adr/0026` 決定4のクロスプロジェクト協調手順は「オープンPRの一覧を取り、各ブランチの
`meta/adr/` 配下をgrepする」と定めるが、その手順ではこの衝突は見えない（FR-019）。

同じブランチは `projects/toyama-dining-radar/**` を1615行にわたって変更しているため、**本改名は
そのブランチと確実に衝突する**。帰結節の最後に扱いを記す。
...
pytest-gremlins の `paths` は**一切動かない**。改名によって import 解決が壊れる経路が構造的に存在
しなくなる。

### 2. シナリオIDプレフィックス `TDR` は据え置く

`TDR` は Toyama Dining Radar の頭字だが、実測693箇所に及び、その中には**承認済み契約の
...
`meta/templates/adr.md` の運用ルール（P-06）は「承認後の本文編集は禁止」と定める。本改名は24本の
...
- **未マージのローカルブランチ `docs/tdr-cs-origin-and-walking-time` と衝突する。** そのブランチは
  `projects/toyama-dining-radar/` 配下に新規ファイル（`adr/0025-...`、`design/explorations/` の
  ラフ3枚とREADME）を追加し、契約4本と `product-brief.md` を変更している。本改名はそのディレクトリを
  丸ごと移動するため、**どちらを先にマージするかを人間が決める必要がある**。先に本改名をマージする
  場合、あちらのブランチは `projects/dining-radar/` 配下へリベースし直すことになる。逆順なら本改名を
  やり直すほうが安い（本PRの中身は機械的な置換なので作り直しの費用が低い）——**後者を推奨する**
- 24本の承認済みADRと契約・reviews・friction-log の本文にプロジェクト名の置換が入るため、本PRの差分は
  行数の割に読む価値が低い。**レビューは「置換以外の差分があるか」に絞ってよい**（置換以外は
  本ADR・`adr/0019` の伏せ字・`meta/scenario-id-prefixes.md` の `TDR` 行・ブランチ名を旧名のまま
  据え置いた3箇所・activeContext・friction-log の FR-018 だけである）
