...
- **同じ族の別の現れ方（2026-09-11、tester が発見して申し送った）**: 役割agentが
  **専用の worktree の中で起動されている**とき、**絶対パスを取るツール（Read/Edit/Write/Grep）に
  worktree の接頭辞を付けないと、主ツリー（別のコミットをチェックアウト中）の内容を読んでしまう**。
  一方 Bash 経由の相対パスは正しく worktree を指す。**同じ「絶対パスで書け」という回復策が、
  worktree の中では逆に誤読を生む**——FR-021 本体で観測された「相対パスで外し、絶対パスで回復する」
  挙動と、方向が逆になっている。
  今回の tester は誤読を自分で検知して直したうえで、次にこの worktree で作業する者への申し送りとして
  報告した。**押し込み先は本体と同じ**——「起動時に渡すカレントディレクトリそのもの」を正すことで、
  相対・絶対のどちらでも同じツリーを指すようにするのが筋であり、「どちらのパスを使え」という規約の
  追記で塞ごうとすると、環境によって正解が反転するためかえって危うい。
...
- **再確認（2026-08-29、独立した2例目）**: デスクトップを地図主役に戻すスライスで、同じエラーが
  **architect 11回・developer 7回**出た（セッション終了時点。追記時は 5回・4回だった——スライスが
  ...
  ことは、これで2セッションで確認されたことになる。**次に触る人は、ブリーフの型や規約の追記で
  直そうとしないこと。** 押し込み先は「起動時に渡すカレントディレクトリそのもの」にある。
  なお developer 側の1件はエラーの形が違った（`File does not exist` ではなく
  `InputValidationError: Read was called with …`）が、相対パスで外して絶対パスで回復する挙動は
  同じであり、同じ族として扱ってよい。

- **同じ原因で、読み取り失敗より危険な形が出た（2026-08-29）**: developer が
  `activeContext.md` の記録を、**指定した作業ツリー（`E:/AWS/run2`）ではなくセッションの主ツリー
  （`E:/AWS/Claude_Workspace/ai-driven-dev-template`）へ書き込んでいた**。131行の追記が、
  PRとは無関係のツリーに未コミットのまま残った。**これまで観測されていたのは
  `File does not exist` という失敗（agent が気づいて絶対パスへ切り替え、回復する）だったが、
  今回は書き込みが成功しているためエラーが出ず、誰も気づかない。** orchestrator が
  マージ後にツリーを同期しようとして `git checkout` が阻まれ、初めて発覚した。
  発覚が遅れていれば、(1) 別スライスのPRに無関係な記録が紛れ込む、(2) 作業が失われたと誤認して
  やり直す、のどちらかが起きうる。
  **これは cause_key の因果の理解を1つ進める**——「初期のカレントディレクトリが主ツリーである」
  という同じ機序が、読み取りでは可視のエラーに、書き込みでは不可視の混入になる。
  押し込み先が「起動時に渡すカレントディレクトリそのもの」にあるという結論は変わらないが、
  **緊急度は上がった**。読み取り失敗は自己回復するが、書き込みは回復しない。
---
...
## FR-026: 承認欄が空のままのADR2本をマージさせてしまい、記録を閉じるだけのPRがまた要った（同じcause_keyの8回目）

```yaml
id: FR-026
date: 2026-08-28
found_at: AI
slice: デスクトップを地図主役に戻す（契約・関所の整備）
agents: [orchestrator]
cause_category: 承認記録が承認行為の時点に依存しており、1本のPRで閉じられない
cause_key: record-update-needs-second-pr
pushed_to:
...
- 事象: `adr/0031`・`adr/0032` を `status: 提案中`・`approved_by:` を
  `<!-- orchestrator が合意を取ってから記入 -->` のプレースホルダのままで PR #166 に載せ、
  「承認をお願いします」と人間に提示した。人間は承認の意思表示として**マージ**した。
  その結果、**承認された2本のADRが「提案中・承認者未記入」のまま main に載った**。
  記録を閉じるためだけのPRがもう1本要った。
- **同じ cause_key の8回目である**（FR-006・FR-008・FR-016・FR-017・FR-018、
  reservation-system FR-015・FR-021）。FR-018 の時点で「規約による是正は6回連続で失敗した」と
  記録されている。
- 原因の仮説: **`meta/adr/0064`（合意はチャットで取り、その場で記録する）を、順番を取り違えて
  適用した。** 0064 は「マージを承認行為にしない」ことを定めているが、orchestrator はそれを
  「承認欄は空のまま出し、承認をもらってから埋める」と読んだ。**承認をもらう手段としてPRを
  差し出した以上、人間の承認行為はマージとして返ってくる**——そのとき記録は既に main にある。
  0064 の意図どおりに運ぶなら、**チャットで裁定を取り、記録を埋め、そのうえで Ready の PR を
  渡す**という順番でなければならなかった。orchestrator は承認を問う設問を
  `AskUserQuestion` で出さず、PR本文と地の文で問うたため、裁定が「マージ」という形でしか
  返ってこない状況を自分で作った。
- 押し込み先: **機械化しない。** `meta/tools/**` は `meta/adr/0046` で AI からの書き込みが
  塞がれており、検査の追加には人間のコミットが要る（FR-017 が同じ理由で未実装のまま）。
  加えて、今回の欠陥は「規約を守り忘れた」のではなく**規程の適用順序を取り違えた**もので、
  検査で捕まえるなら「main に `提案中` の ADR が存在しないこと」という別種の検査になる。
  **代わりに手順を1つ変えた**——承認が要る成果物は、(1) `AskUserQuestion` でチャットの裁定を
  取り、(2) 裁定を記録に埋め、(3) そのうえで Ready の PR を渡す、の順に固定する。
  この順なら人間のマージは常に「公表」であり、記録は渡す前に閉じている。
- 補足: **8回目にして初めて、原因が「守り忘れ」ではなかった。** FR-016〜FR-018 は
  「書き方を知っていて使わなかった」であり、今回は「新しい規程の適用順序を誤った」である。
...
## FR-028: 旧ラフ2枚の食い違いを「矛盾の解消」として片側採用し、消した側が人間の要件だった

```yaml
id: FR-028
date: 2026-08-30
found_at: 人間
slice: 会スコープの取り込み（ブリーフ改訂とdesigner正規描き直し）
agents: [architect, designer, orchestrator]
...
- 事象: designer の正規描き直し後、人間が「日程と店舗を同時に決めることもあるので、その導線が
  あるか確認してほしい」と後追いで指摘した。旧ラフA（issue #104 案A由来）の中心価値は「日程候補と
  店舗候補を1画面に並べ、突き合わせて決めきる」ことだったが、ブリーフ§2の状態機械（旧ラフF由来の
  逐次）と食い違っており、designer はこれを「3局面を1枚に重ねた矛盾」として解消し（変えた点2として
  正直に申告はしていた）、逐次だけが残った。プレビュー導線の追加に1往復を要した。
- 原因: 旧ラフの F板（逐次フロー）と A板（同時表示）が最初から逆のことを描いており、その同時性が
  **要件なのか作画の粗なのかがどこにも言語化されていなかった**。architect のブリーフ改訂も designer の
  描き直しも F板を正として扱い、A板の同時性は誤りとして閉じられた。designer は「どちらとも書いて
  いない」型の穴（D7）を判断点として人間に上げることはできたが、**片方を正とすれば筋が通ってしまう
  「2枚が逆のことを描いている」型の穴は、矛盾として解消できてしまうため、判断点の網（D1〜D7）に
  掛からなかった**。変えた点の列挙（9件）に含まれていたので人間は発見できたが、発見の負担が人間側に
  残った。
- 押し込み先の候補（未実施）: architect / designer の役割定義または雛形に「入力素材どうしが食い違う
  場合、片方を正として解消するのは、どちらが正かを人間が既に裁定している場合に限る。裁定が無いなら
  食い違いそのものを判断点として列挙する」を足す。`meta/**` は共有契約なので、起こすなら ADR と
  直列化が要る（meta/adr/0026）。今回は「変えた点として申告する」という既存の規約が発見可能性を
  担保した——押し込みの要否は、同型の再発を待ってから判断してもよい（P-05の精神）。
- 補足: 同セッションの D1〜D7 の網自体は機能した（7件が選択肢のまま人間に届き、8件目の同時決めも
  変えた点の申告経由で1往復で回収された）。摩擦の核は「静かに解消された」ことではなく、**解消の
  申告が『変更の記録』の形をとり、『あなたの判断が要る点』の形をとらなかった**ことにある。

---

## FR-029: 契約だけを先にマージする運用が、必須フィールドを足した瞬間に成立しなくなった

```yaml
id: FR-029
date: 2026-09-03
found_at: L4
slice: 会フロー後半（店の絞り込み連携・承認投票・確定）の統合契約
agents: [orchestrator, architect]
cause_category: 承認の順序と機械検査の前提が噛み合っていない
cause_key: contract-first-merge-breaks-response-schema-gate
pushed_to: []
status: 未対応
principles: [P-02, P-08]
```

- 事象: 契約のみを変更したPR #184（`src/**`・`tests/**`は無変更）で、**L4が赤になった**。
  `jsonschema.exceptions.ValidationError: 'votingStartedAt' is a required property`——`Gathering`に
  新しい必須フィールドを足した契約に対し、稼働中の実装がそれを返していないため、既存の
  受け入れテスト（`assert_matches_openapi_schema`が実応答を契約で検証する）が落ちた。
- **これは検査の誤作動ではなく、検査が正しく仕事をした例である。** 「承認済みの契約と実物が
  食い違っている」ことを、人間が気づく前に機械が指摘した——`ADR-0023`のときに実際に起きた
  「承認済み文書が実装と食い違ったまま一定期間放置された」問題の、機械的な再発防止として働いた。
- 原因: このプロジェクトの承認点①（契約と設計骨格）は「契約PRを先に人間がマージし、その後
  実装スライスへ進む」という順序を前提にしている。一方L4は実応答を契約で検証する。**応答スキーマに
  required フィールドを足す種類の契約変更は、この2つの前提を同時に満たせない**——契約だけを
  マージするとmainが赤になり、`render.yaml`の`autoDeployTrigger: checksPass`によりデプロイも
  止まる。今回は契約と実装を同じPRで着地させる形に切り替えて回避した（人間へ報告済み）。
- 押し込み先の候補（未実施）: (a) 「応答スキーマに required を足す契約変更は、実装と同一PRで
  着地させる」を`meta/`のスライス手順に明記する、(b) 契約PRの段階では新フィールドを optional で
  入れ、実装スライスで required へ格上げする2段階運用にする——ただし(b)は「契約は実装より先に
  厳密であるべき」という本リポジトリの立場と衝突するため、採るなら理由の明文化が要る。
  (c) L4のスキーマ検証を「契約に無いフィールドを禁じる」方向だけに緩める——検出力が落ちるため
  推奨しない。**判断はまだしていない**。
- 補足: 同種の食い違いは`meta/adr/0013`が確立した「承認済み画面が契約の観測面を駆動する」順序
  （画面→契約→実装）とは別物である。あちらは契約の**書き順**の話、こちらは契約の**着地単位**の話。

---

## FR-030: 新しい画面状態に横断検査を呼び忘れる抜けが、前回直したばかりで再発した

```yaml
id: FR-030
date: 2026-09-04
found_at: 人間
slice: 会フロー後半（店の絞り込み・承認投票・確定）
agents: [tester, reviewer]
cause_category: 検査の網羅が個々の呼び出し忘れに依存している
cause_key: cross-cutting-check-not-wired-to-new-screens
pushed_to: []
status: 未対応
principles: [P-08]
```

- 事象: reviewerの独立監査が、横断検査 `screen_has_no_forbidden_controls_or_disclosures`
  （禁止された操作面・開示の検査）が**新設された6つの画面状態のどれにも呼ばれていない**ことを
  指摘した（Major#1）。新しいpurpose値を`GATHERING_ALLOWED_PURPOSES`の一覧へ足すところまでは
  行われており、**「一覧を更新する」という目に見える作業は済んでいたが、実際に呼ぶのを忘れていた**。
- **これは前ラウンドの監査Major#3とまったく同じ族の抜けである。** 直した箇所には
  `test_gathering_scheduling_acceptance.py` に「Reviewer audit Major#3」というコメントが残って
  いる——つまり**同じ人が、同じ理由で、直した直後の次のラウンドで再発させた**。
- 原因の仮説: この横断検査は「画面状態ごとに1回ずつ手で呼ぶ」設計であり、**網羅性がテスト作者の
  記憶に依存している**。新しい画面状態が増えても、呼び忘れても、何も赤くならない——
  `allowedPurposes` の一覧更新のほうは「値を足さないと既存テストが落ちる」ため強制力があるのに、
  呼び出しのほうには強制力が無い。**同じ契約の2つの側面のうち、片方だけに機械的な関所がある。**
- 押し込み先の候補（未実施）: (a) 画面状態を列挙するデータ構造（テストIDの組と到達手順）を持ち、
  そこから横断検査を自動で回すヘルパーを作る——新しい状態を足す作業が「一覧に足す」という
  強制力のある形になる。(b) L2（構造テスト）で「契約の各namespace／各画面状態が、
  少なくとも1つのテストから横断検査を受けていること」を機械的に検査する。
  **(b)のほうが本命に見える**——検査の呼び忘れを検査する層はL2の役割に近く、テスト作者の記憶に
  依存しない。ただし「画面状態」を機械が数える定義が要る（契約側にその列挙が必要かもしれない）。
- 補足: 2回とも**reviewerの独立監査が検出した**——役割分離が機能している証拠でもある。ただし
  監査は毎回人間の指示で走らせており、機械的な関所ではない（`meta/agents.md`§4 step 7 は
  「必要なら実施」の書き方で、PRテンプレのチェック欄は自己申告）。**3回目が起きるなら、
  押し込みを実施する判断をすべきである**（FR-008が同種の判断で用いた「5回目」より早い理由は、
  この検査が公開面の禁止事項——非認証の参加者画面に何が出てはいけないか——を守る位置にあるため）。

---

## FR-031: 部分編集を指示した相手が全文再構成を行い、既存の4シナリオを消したうえで「元から無かった」と報告した

```yaml
id: FR-031
date: 2026-09-05
found_at: AI
slice: 店の投票を三段階にする（実機フィードバック対応）
agents: [architect, orchestrator]
cause_category: 全文再生成が、消したことに気づけない削除を生む
cause_key: full-rewrite-silently-drops-content
pushed_to: []
status: 未対応
principles: [P-01, P-08]
```

- 事象: `contracts/gathering-scheduling.feature` の TDR-GTH-34 に1行足すだけの依頼に対し、architect が
  ファイルを**全文再構成**した結果、**TDR-GTH-38〜41 の4シナリオが消えた**。しかも本人は消したことに
  気づかず、完了報告で「**TDR-GTH-38〜41 はそもそもファイルに存在しない**——ブラウザ契約と
  test-support がそれらを参照しているのは既存の齟齬だ（FR-028として報告）」と書いた。
  **自分の編集で生じた欠落を、他人が作った既存の欠陥として報告している。**
- orchestrator が `git show <commit>:<path>` で直前のコミットを確認したところ4件とも存在しており、
  作業ツリーの側だけが失っていた。復旧は `git checkout --` で戻し、意図された1行の追加だけを
  `python` の文字列置換（前後の一致を `assert` で固定）で入れ直した。所要2分。**gitが無ければ
  静かに失われていた。**
- 原因の仮説: (1) このリポジトリの契約ファイルは1000〜2000行あり、**全文を書き直すと出力の途中で
  内容が落ちても書いた本人には差分が見えない**。(2) architect の役割には Edit/Bash が無く
  `Write`（全文置換）しか持たないため、**構造的に全文再構成へ誘導されている**——直前の担当が同じ
  ファイルの全文再構成でレート制限に達して中断したのも同じ根である（同日、別セッション）。
- **orchestrator 自身の落ち度も記録する**: 依頼文に「全文再生成はしないこと、対象箇所を絞った
  部分編集で」と書いたが、**相手が持つ道具では部分編集ができない**ことを確認していなかった。
  役割の道具立てを見ずに作業のやり方を指示した。
- 押し込み先の候補（未実施）: (a) `.claude/agents/architect.md` に Edit を足す——部分編集ができれば
  全文再構成の必要が消える。ただし role 定義は共有契約（`meta/agent-runtime-mapping.md`）なので
  ADR と直列化が要る。(b) orchestrator 側の受け入れ検査として、契約ファイルを触った直後に
  **`git diff --stat` の削除行数を必ず読む**——今回もそれで発見した。(c) L0 govlint に
  「`.feature` のシナリオIDが、他の契約が `verifiesScenarios` で参照する集合を包含すること」の
  検査を足す——**参照だけが残って実体が消える形の欠落を機械が捕まえられる**。`meta/tools/**` は
  `meta/adr/0046` によりロックされており人間の解錠コミットが要る。
- 補足: FR-028（食い違いを解消せず報告せよ）は正しく働いた——architect は勝手に辻褄を合わせず
  報告した。**報告する規律はあったが、報告内容の事実確認は orchestrator 側でしか行えなかった。**

---

## FR-032: 修正を入れたファイルを誰も読んでおらず、「直した」と報告した修正が一度も実行されていなかった

```yaml
id: FR-032
date: 2026-09-08
found_at: AI
slice: 三段階投票ラウンドの間欠失敗の追跡
agents: [developer, orchestrator]
cause_category: 修正が届いているかを確かめずに完了とした
cause_key: fix-landed-in-a-file-nothing-loads
pushed_to: []
status: 未対応
principles: [P-01, P-08]
```

- 事象: 受け入れテストの間欠失敗を追った developer が、**メモリ上のSQLiteで Django が1本の接続を全スレッドで共有する**という実バグ（savepointの破損）を突き止め、`settings_acceptance.py` を直した。全件を3回連続で緑にして報告し、**orchestrator は「真因を特定・修正した」と人間へ報告した**。
- **しかし症状は残った。** 次の担当が調べたところ、**`manage.py test`（`--settings` 無し）も pytest も `settings_test.py` を読んでおり、`settings_acceptance.py` は誰も選んでいなかった**——修正は一度も実行されていない。前任が「3回連続で緑」を得たのは、明示的に `--settings=dining_radar.settings_acceptance` を付けて走らせていたためで、**CI と既定のコマンドが使う経路とは別の経路を検証していた**。
- **orchestrator の落ち度**: 完了報告の「3回連続で緑」を、**どの経路で走らせたかを確かめずに受け入れた**。さらに、その報告をもとに人間へ「真因を特定して修正した」と伝えた——**実際には直っていなかった**。人間には後で訂正した。
- 原因の仮説: (1) このプロジェクトには `settings.py`／`settings_test.py`／`settings_acceptance.py` の3つがあり、**どれが実際に使われるかがファイル名から自明でない**。(2) 検証の実行コマンドを担当者が自分で選べるため、**CI が使う経路と違う経路で緑を得ても気づけない**。
- 押し込み先の候補（未実施）: (a) **完了報告に「実行したコマンドの全文」を必ず書かせる**——orchestrator が経路の違いに気づける。今回の依頼文は「全件を3回連続で実行」としか書いておらず、コマンドを指定していなかった。(b) `settings_acceptance.py` を `settings_test.py` の薄い再輸出にする（次の担当が実際にそうした）——**設定が2箇所に分かれている限り同じ事故は再発する**ため、統合そのものが本命。(c) CI と同じコマンドを1つのスクリプトに固定し、検証はそれだけを使わせる。
- 補足: この探索自体は無駄ではなかった——**接続共有によるsavepoint破損は本物のバグ**であり、修正先を正しい場所へ移して初めて効いた。問題は「直した」と言い切った時期が早かったことである。

---

## FR-033: 製品の画面を、テストを通すための都合で決めていた

```yaml
id: FR-033
date: 2026-09-12
found_at: 人間
slice: 会の画面群・実機フィードバック（第2回）
agents: [developer]
cause_category: 検証の都合が製品の判断を乗っ取る
cause_key: test-convenience-decides-product-ui
pushed_to: []
status: 未対応
principles: [P-02, P-04]
```

- 事象: 会をつくる画面のカレンダーが**3か月同時表示**になっている。人間から
  「なぜ3か月？ 入れると改修が増えるから嫌ってるだけじゃない？」と問われた。
- 実際の理由: `TDR-GTH-46`（候補日の一括登録、1件でも重複すれば全体拒否）が **+3日と+20日を同じ
  バッチで登録する**ため、月末に実行すると月をまたぐ。1か月表示ではこの検査が通らない。
  だから `showMonths: 3` にした。**改修を嫌ったのではなく、テストを通すために選んだ。**
- なぜ摩擦か: 動機は違っても、**利用者を見ずに画面を決めた**点は同じである。人間の指摘は当たっている。
  3か月ぶんの日付格子が一度に出る画面は、日程を数日選びたい人にとって選びにくい
  （「おおよそ選べたものではない、とても使いにくい」）。
- 本来の筋: テストが月をまたぐ必要があるなら、**テスト側が月を送る操作を持てばよい**
  （flatpickr の prev/next は動く）。UI の既定表示を検査の都合に合わせるのは順序が逆である。
- 押し込み先の候補（未決）: (a) 役割契約に「製品の既定値をテストの都合で決めない。テストが届かない
  なら**テストが操作を増やす**」を書く。(b) この1件に限れば、カレンダーの表示月数はデザイナーの板が
  決める値であり、次の描き直しで決着する。

---

## FR-034: 承認のために書いた説明文を本番画面へ置き、それがレイアウトを壊した
...
cause_category: 承認材料と製品の混同
cause_key: approval-prose-shipped-as-product-ui
pushed_to: []
status: 未対応
principles: [P-04, P-06]
```

- 事象: 人間から「**全般的にユーザの承認を得るための文書を本番の画面に埋め込むのはやめてください**」
  と明示指示。名指しされたのは参加者の回答画面の3つ——
  「ほかの人の回答も見えています」（`VISIBILITY_HINT`）、
  「ここまでの回答は保存されています。またあとで、続きから答えられます。」、
  「幹事から届いたリンクで開いています。ログインも名前も要りません。名前はあとからでも付けられます。
  答えは何度でも変えられます。」（`renderFinePrint`）。
- **単に見苦しいだけでは済んでいない。**2つ目の確認文は `gth-foot`（`display: flex`）の中に
  ボタン2つと**兄弟として**入る。文が横幅を取るため「あとで答える」「結果をのぞく」が縦一列に潰れ、
  人間から「レイアウト崩れてる」と報告された。**承認のために置いた一文が、ボタンを壊していた。**
- 原因: 契約が `requiredOutcome` を持つ操作（可視性の反転、answerLater の確認、参加者リンクの性質）を、
  **可視の散文で満たそうとした**。本来これらは機械観測（`data-testid`・`data-*-purpose`・
  tally 要素の存在）で満たすものであり、利用者に読ませる文ではない。
- 押し込み先の候補（未決）: (a) 役割契約（developer）に「画面に文を足す前に『利用者が行動するために
  要る文か、実装を示したい文か』を問う。後者はテストの assert に置く」を書く。
  (b) 契約側でも、`requiredOutcome` を散文の表示で満たしてよいのかを architect が明示する。

---

## FR-035: 描画不変量の関所が、このラウンドで新設した画面群に一度も掛かっていなかった
...
slice: 会の画面群・実機フィードバック（第2回）
agents: [orchestrator, developer]
cause_category: 関所の適用範囲が宣言されていない
cause_key: gate-scope-not-extended-to-new-screens
pushed_to: []
status: 未対応
principles: [P-04, P-06]
```

- 事象: FR-034 の潰れたボタンは、**この製品が既に持っている関所で捕まえられたはずだった。**
  `tests/ui_invariants` の 44px 検査は幅と高さの**両方**を測る（`assertGreaterEqual(box["width"], 44)`）。
  潰れたボタンの幅はこれを確実に下回る。
- なぜ通ったか: **`tests/ui_invariants` は店を絞る画面にしか掛かっていない。**会の画面群
  （幹事の一覧・作成・会の詳細・参加者の回答）に対しては一度も実行していない。ファイル内に
  `gathering` の文字列が1つも無いことで確認した。
- 構造: `adr/0020` はこの層をこの製品の実証の場として新設したが、**新しい画面群を足したときに
  この層へ繋ぐ手順がどこにも書かれていない。**L4（受け入れ）は契約のシナリオが増えれば自動的に
  増えるが、L5 は対象画面を人が列挙する作りなので、増やし忘れが静かに通る。
- 押し込み先の候補（未決）: (a) `tests/ui_invariants` を会の画面群へ広げる（このラウンドで着手する）。
  (b) 画面を新設する契約スライスのチェック項目に「L5 の対象へ追加したか」を足す。
  (c) 対象画面の列挙を契約（`*-browser-interface.yaml` の `renderModes`）から機械的に導く。

---

## FR-036: 契約が画面間の行き来を扱わないまま実装し、行き止まりを量産した
...
found_at: 人間
slice: 会の画面群・実機フィードバック（第2回）
agents: [architect, designer, developer]
cause_category: 契約が扱わない領域の放置
cause_key: contract-silent-on-navigation-produces-dead-ends
pushed_to: []
status: 未対応
principles: [P-02, P-06]
```

- 事象: 人間から「会をつくる画面から戻るすべがない、**ページの構成がそもそも考えられていない**」
  「店を選んだあとに確定とか戻るフローがないから手動で開き直さないといけない」と報告。
- 確認した事実: **会の画面群に「戻る」が1つも無い。**`src/dining_radar/gathering/` と
  `src/dining_radar/web/` を検索して該当ゼロ。作る画面にも、会の詳細にも、会モードの候補検索画面にも
  存在しない。会モードから会へ戻る導線が無いため、店を選んだあとブラウザの操作で開き直すしかない。
- 原因: 契約（`.feature` / `-browser-interface.yaml`）は**1画面の中で何が観測できるか**は厚く定めるが、
  **画面から画面への行き来**をほとんど扱わない。シナリオは毎回その画面を直接開くところから始まるため、
  行き来が無くても全シナリオが通る。デザイナーの板も6枚が個別に描かれており、板と板の間の線は無い。
  **誰も担当していない領域**として落ちた。
- 押し込み先の候補（未決）: (a) デザイナーの板に「画面のつながり（どこから来て、どこへ戻るか）」を
  必須の1枚として要求する。(b) 契約に画面遷移の Must を置くか、置かないと決めるなら
  「行き来は実装裁量」と明記して裁量であることを可視にする。

---

## FR-037: 並列の developer 3人に持ち場を割ったら、持ち場をまたぐ不変条件が3つ割れた
...
slice: 会の画面群の描き直し（統合）
agents: [orchestrator, developer]
cause_category: 持ち場の境界が不変条件を横切っている
cause_key: parallel-owners-split-a-shared-invariant
pushed_to: []
status: 未対応
principles: [P-06, P-04]
```

- 事象: 会の画面群の実装を、ファイルが重ならないように developer 3人（バックエンド／幹事画面／
  参加者と候補の画面）へ割って並列で動かした。**4本（テストを含む）とも git の上では衝突なく
  マージできた。**それでも統合して初めて、次の3つが割れていた。
  1. **共有区間のバイト一致が崩れた。**`gathering.js` と `participant.js` は日時の書式化の区間を
     バイト単位で同一に保つ単体テストを持つ。幹事画面の担当がその区間に関数を足し、区間の説明文も
     書き換えた。参加者画面の担当は触っていない。**両ファイルは別々の持ち場だったので、どちらの担当も
     もう片方を直せなかった。**
  2. **上部ナビが画面によって違った。**幹事画面の担当は「ランチ候補をさがす」「ランチ会」を常設する部品を
     作ったが、候補画面（`web/`）は持ち場の外だった。候補画面の担当は入口の文字が消える CSS だけを直し、
     ラベルは「会」のまま、「ランチ候補をさがす」も足さなかった。**第1束の裁定は「すべての画面に2つ」**である。
  3. **描画不変量の関所が新しい操作に掛からなかった（FR-035 の再発）。**`tests/ui_invariants` は
     幹事画面の担当の持ち場で、その担当は自分の新しい操作だけを足した。参加者画面と候補画面の新しい操作は
     持ち場の外だったので、誰も足さなかった。
  あわせて、参加者画面の担当は日程の設問を**1問ずつ**出す既存の作りを残した。契約はそれを許すが、
  第2束の板は**全部を同時に出す**設計だった。
- 原因: **ファイルの重なりで持ち場を割ったが、不変条件はファイルをまたいでいた。**
  「同じ区間を2ファイルで同一に保つ」「全画面に同じナビを置く」「新しい操作は全部関所に掛ける」は、
  どれも**1つの持ち場の中では完結しない**。ファイルが重ならないことは、衝突しないことの保証にはなるが、
  整合することの保証にはならない。
- 実害: 単体テスト1件の赤、画面間のナビの食い違い、関所の穴。**統合の試しマージを先にやったので、
  PR を出す前に見つかった。**受け入れテストもこの統合で初めて6件落ちた（うち実装側4件）。
- 対処（今回）: 統合後の修正は**画面側を1人にまとめて**持たせた。
- 押し込み先の候補（未決）:
  (a) 並列に割るときは、**ファイルではなく不変条件で**持ち場を切る。またぐ不変条件（共有区間・共通部品・
      関所）には1人の持ち主を決め、他の担当には「触らずに報告する」とだけ書く。
  (b) orchestrator の手順として、並列の developer を回したら**全員が終わる前に統合の試しマージと単体テストを
      回す**ことを必須にする（今回それで見つかった）。

---

## FR-038: 緑の受け入れテストのうち、Given が薄くて欠陥を見分けられないものがあった

```yaml
id: FR-038
date: 2026-09-13
found_at: AI
slice: 会の画面群の描き直し（統合後の欠陥注入）
agents: [tester, orchestrator]
cause_category: 検査が欠陥を見分けられない
cause_key: given-too-thin-to-discriminate
pushed_to: []
status: 未対応
principles: [P-04]
```

- 事象: 統合後に全層が緑になった状態で、10件の欠陥を実装に1つずつ注入し、対応する受け入れテストが落ちるかを確かめた。
  **8件は狙った検査で落ちたが、4件は通った。**うち2件は Given が薄すぎて、欠陥が画面に表れようがなかった。
  1. **TDR-GTH-49（幹事の回答表）**: 参加者リンク2本が**別々の候補日に1つずつ**答える Given だった。
     どの行にもセルが1つしかないので、「行の中で候補日どうしの答えを入れ替える」欠陥を入れても何も変わらず、通った。
     検査そのもの（リンクID→{候補日ID: 答え} の完全一致）は強かった。
  2. **「あとで答える」の確認**: 候補日が1つで、しかも答え済みの Given だった。
     「未回答の候補日も再掲に描く」欠陥を入れても、未回答の候補日が存在しないので通った。
- 残り2件の内訳（Given の問題ではない）: 確定後の地図の線の数は「実際に描いたもの」ではなく
  「決まった関数を通ったか」を数えており、別の書き方で描いた線を捕まえられなかった（実装の数え方を直す）。
  0票の最有力の検査は、注入した行が早期 return の後ろにあって到達しておらず、検査の効き目を証明できていなかった（注入し直す）。
- 原因: tester は**シナリオの文を満たす最小の Given**を書く。シナリオ「参加者ごとに、どの候補日へ何と答えたかが一覧で示される」は、
  1人1日でも文面上は満たせる。**しかし「満たす」ことと「壊れた実装と区別できる」ことは別である。**
  assertion の強さだけを監査しても、Given の薄さは見えない。**独立監査もこの2件を強い検査と判定していた。**
- なぜ見つかったか: **欠陥注入を実際に回したから。**監査の読み下しでは見つからなかった。
- 押し込み先の候補（未決）:
  (a) tester の役割契約に「Given は、そのシナリオが否定する実装を1つ具体的に想定し、それが**画面に表れる**状態を作ること」を書く。
  (b) 受け入れテストの新規・変更は、統合後に欠陥注入を回すまで完了としない、を手順にする（今回それで見つかった）。

---

## FR-039: URL が変わっただけで画面の描画を待ったことにし、描画前の DOM を数えていた（2回目）

```yaml
id: FR-039
date: 2026-09-13
found_at: AI
slice: 会の画面群の描き直し（統合後の全体実行）
agents: [developer, orchestrator]
cause_category: 検査の同期が描画を待っていない
cause_key: url-wait-before-client-render
pushed_to: []
status: 未対応
principles: [P-04]
```

- 事象: 統合後の全体実行で、描画不変量の `test_e_gathering_dashboard_controls_meet_44px_minimum_target` が
  `0 not greater than 0 : no activatable controls with a declared purpose were found (SCHEDULING at phone-390x844)` で落ちた。
  **単体で3回回すと3回とも通った。**
- 原因（再現して確定）: 会をつくる補助関数が、送信後に**URL が会の画面に変わることだけ**を待って戻っていた。
  会の画面は JavaScript で会を取得してから操作要素を描くので、URL が変わった時点ではまだ要素が無いことがある。
  直後に呼ぶ `Locator.count()` は**自動で待たない**（Playwright の中で `count()` だけが即時に数える。
  `get_attribute()`・`bounding_box()` は要素の出現を待つ）。全体実行でマシンが忙しいと描画が遅れて競合に負けた。
  **会の画面の描画を2秒遅らせると、必ず同じメッセージで落ちることを再現した。**
- 対処: URL ではなく、**契約が「日程を聞いている局面で必ず存在する」と定める要素**（候補日の一覧の項目）が
  見えるまで待つようにした。数える側も、少なくとも1つ現れるまで待ってから数える。
  2秒の遅延を入れても通り、遅延なしで5回連続緑。
- **2回目である。**受け入れテストの会の削除（TDR-GTH-48）でも、画面自身の遷移が終わる前に次の操作を
  出して `net::ERR_ABORTED` で間欠的に落ちていた。形は違うが原因の型は同じ——
  **「ナビゲーションが確定した」ことと「画面が描き終わった」ことは別であり、前者だけ待って後者を前提にした。**
- なぜ間欠的に通っていたか: 単体で回すと速いので、たまたま競合に勝つ。**「単体では通る」は、直ったことの証拠にも、
  問題が無いことの証拠にもならない。**人間の指示「原因を特定してから出す」に従って、単体の再実行で片付けなかった。
- 押し込み先の候補（未決）:
  (a) 受け入れテストと描画不変量の DSL に「画面を開く／遷移させた後は、その画面で必ず存在する要素を待ってから読む」
      補助関数を1つ置き、URL だけを待つ書き方を使わない。
  (b) `Locator.count()` を、直前に自動待機の無い場所で呼ばない、を検査の書き方の規則として残す。

## FR-040: 退役させた Given の作り方を、新しいシナリオでもう一度使った（標本抽出で店が画面から消える）
...
cause_key: retired-test-technique-reused
pushed_to: []
status: 未対応
principles: [P-04]
```

- 事象: TDR-CS-22 の Given が、`/candidate-proposals` を API で1回呼んで店を1件会に入れ、その後に画面を開いていた。
  画面は `/candidate-proposals` を**別に**呼び直す。契約は、表示上限（5件）を超える母集団では標本抽出が
  リクエストごとに非決定的だと定めている。木・金・土（6件）の日付では、先に入れた店が画面に出ないことがある。
- 発見: 独立監査（reviewer）が Major として指摘した。テストは tester の手元でも統合後の全体実行でも緑だった。
- 原因（再現して確定）: 母集団6件の日付で、Given の2回呼びを API で30回繰り返した。先頭の候補を選ぶと
  0/30 で消えなかった（この合成データでたまたま安全な位置だった）。末尾の候補を選ぶと **13/30 で消えた。**
  **この失敗の型は、別のシナリオで一度再現して直し、その書き方を退役させていた**
  （`dsl/gathering_scheduling_browser.py` の `fetch_confirmed_date_open_shop_ids_with_a_spare` の docstring）。
  退役の記録は DSL の docstring にしか無く、別の DSL ファイルを書く tester には届かなかった。
- 対処: Given の開催日を、母集団が上限以下（水曜、4件）の日に固定した。末尾の候補を選ぶ最悪の条件でも
  0/30。会に入れていない候補は3件残るので、欠陥を見分ける力は落ちていない（FR-038）。
- なぜ緑だったか: たまたま安全な位置の候補を選んでいたので、**緑は正しさの証拠になっていなかった。**
  FR-038（Given が薄い）・FR-039（待ち方）と同じく、読み下しの監査か状態を変えた再現でしか見えない。
- 押し込み先の候補（未決）:
  (a) 「会モードの画面と API を別々に呼ぶ Given は、母集団が表示上限以下の日付に固定する」を、
      DSL 共通の補助関数（開催日を選ぶ関数）に閉じ込め、個別の Given で日付を選ばせない。
  (b) 退役させた検査の書き方を、docstring ではなく tester が起動時に読む場所（step の書き方の規則）に残す。
