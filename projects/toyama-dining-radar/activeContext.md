...
1. Obtain human review and merge of the contract-only candidate-search PR, including the draft browser interface and acceptance-only test-support contract.
2. After that merge, implement the approved candidate-search slice: server-side HTTPS Hot Pepper adapter, deterministic recommendation, authenticated proposal endpoint, candidate cards, Leaflet/OSM map, re-proposal modal, and safe failure handling.
