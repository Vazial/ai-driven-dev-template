...
The current feature branch contains the chat-approved filter-model UI/UX refinement after `955e10d`: the map-led candidate surface has a bottom-overlapping, horizontally swipable card deck; mobile removes the double outer card chrome; and filter controls separate applied conditions from pending changes. The filter labels reflect the soft-filter model: confirmed card-payment unavailability is excluded while information unavailable remains, and budget controls explicitly say they use dinner-budget tiers. Developer L1 (215 tests, 96% coverage), L2 (7 tests), and L3 (120 tests plus both Django checks) are green. L4 is not green: its existing `955e10d` suite cannot collect because of an undefined helper import. The orchestrator completed L5/control-surface checks using the human-owned `.env.local` and real data at 375×812 and 1440×900: real-data retrieval, map-led deck, card↔pin synchronization, horizontal card movement, dirty-filter behavior, 44px minimum targets, no privacy-sensitive text, same-origin scripts, and readable OSM attribution were observed. The check found and the branch corrected Leaflet stacking over the deck and attribution occlusion. A later responsive pass measured no page-level horizontal or vertical overflow in the default collapsed-filter state at either viewport, fixed mobile cards to 303px wide and at most about 293px tall in the observed real-data set, kept all expanded-filter targets at least 44×44px, and brought the full PC cards and provider credit above the 900px fold. The horizontal overflow that remains is intentionally confined to the agreed card deck; its scrollbar is hidden on touch widths and retained on PC for mouse discoverability.

The isolated `design-preview` now also contains a review draft for the next mobile-first placement pass. It keeps synthetic data and no network calls, while previewing a 52px header, one-line condition toolbar, horizontally scrolling single-row filter categories, a map-filling decision surface, a compact full-card carousel with an explicit `1 / 5` counter, and progressive disclosure that omits the low-value description from the initial mobile card. This draft is not yet human-approved and has not been translated into the Django production screen. At 375×812 the collapsed preview measured exactly one viewport with a 343×about-209px card and 44px minimum controls; at 1440×900 it also measured one viewport without page overflow.
...
1. Reconfirm current Hot Pepper and map-provider terms before public operation, especially credit, schema, and caching. A change to the no-history/no-durable-identifier product policy requires a new human decision before work starts.
2. Consider whether ADR-0003's stated design-preview stack (React, TypeScript, Tailwind, shadcn/ui) should match the receiver's actual dependencies (React, TypeScript, `lucide-react` only, with hand-written CSS), by installing the missing packages or amending the ADR. The divergence is unresolved.
3. `meta/tools/govlint.py`'s `SCENARIO_ID` pattern cannot match `TDR-CS-01` or `TDR-AUTH-01`, so all 19 TDR scenario IDs have never been checked by L0. Fixing it needs a human unlock commit for `meta/tools/**` (`meta/adr/0046`).
