...
The implementation is four modules. `recommendation` is pure Python with no framework or provider dependency: it deduplicates by provider page URL and builds up to four concepts (`PROXIMITY`, `GENRE_FOCUS`, `NON_SMOKING_REFERENCE`, `IZAKAYA_BAR_INCLUDED`), offering each only when it is explainable from the current candidates (ADR-0004 decision 1). `integrations/hotpepper` is the HTTPS-only adapter with env-based configuration, query-key redaction, and provider-shape normalization. `suggestions` mediates the fresh search and the pipeline, applies per-organizer rate limiting, and owns the acceptance-only state seam; it is also the only path by which `web` may reach the provider adapter, which a structural test enforces. `web` serves `POST /candidate-proposals`, a serializer matching the API schema exactly, and the authenticated screen, whose candidate surface is rendered client-side by vanilla JavaScript with no bundler.
...
1. **Replace the concept-lens model with filters plus "try again."** The human judged on 2026-08-10 that the lens abstraction no longer earns its place. Every surviving lens decomposes into a filter or a sort: `PROXIMITY` and `NON_SMOKING_REFERENCE` are sorts, `GENRE_FOCUS` is a filter plus a sort, and `IZAKAYA_BAR_INCLUDED` is the inversion of a default-on filter. The requested controls are genre, an izakaya/bar inclusion toggle (default excluded), non-smoking, credit-card acceptance, budget, and a nearest-first sort. Total seats stays a display-only card field and does not become a filter. Unknown values are shown as unknown and ranked last, never filtered out. Filtering must apply to the fetched population, not to the already-capped five. The human also asked for some randomness, because the same shops appear on every run.
2. Reconfirm current Hot Pepper and map-provider terms before public operation, especially credit, schema, and caching. A change to the no-history/no-durable-identifier product policy requires a new human decision before work starts.
