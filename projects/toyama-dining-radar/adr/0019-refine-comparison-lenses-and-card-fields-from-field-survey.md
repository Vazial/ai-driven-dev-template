...
**（2026-08-10 追記。`adr/0023`、提案中・未承認のドラフト）** 決定8が定めた「可視ラベルには必ず
『ディナー』という語を含める」というMustは、`adr/0023`（決定10）により**適用単位をカード単位から
画面単位（1箇所の注記）へ変更する**提案を受けている——カードの可視値は段階語（低/中/高）だけになり、
「ディナー予算をもとにした目安である」という開示と各段階のおおよその金額は、カードごとに繰り返さず
画面に1箇所の注記へ集約する。これは人間の明示的な判断（「ディナー目安〇〇円じゃ何のために載せてるの
かわからない」「高中低くらいでいい」）を受けたものである。決定8の他の内容——`dinnerBudgetTier`が
`LOW`/`MID`/`HIGH`の3値であること、しきい値（〜2,000円／2,001〜4,000円／4,001円〜）、`ConceptKind`
や並べ替え基準にしないこと、ランチ価格を推論・断定しないこと——はいずれも`adr/0023`の対象外であり、
そのまま有効。`adr/0023`はまだ人間の承認を得ていないドラフトである——承認されるまでは本ADR決定8は
現行の契約（`candidate-search-api.yaml`・`candidate-search-browser-interface.yaml`の該当バージョン）
どおり有効であり、この追記はその事実を変えない。詳細と判定根拠は`adr/0023`を参照する。
