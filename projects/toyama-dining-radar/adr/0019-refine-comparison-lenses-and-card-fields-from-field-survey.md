...
**（2026-08-10 追記。`adr/0020`、提案中・未承認のドラフト）** 決定8が定めた「可視ラベルには必ず
『ディナー』という語を含める」というMustは、`adr/0020`（決定10）により**適用単位をカード単位から
...
や並べ替え基準にしないこと、ランチ価格を推論・断定しないこと——はいずれも`adr/0020`の対象外であり、
そのまま有効。`adr/0020`はまだ人間の承認を得ていないドラフトである——承認されるまでは本ADR決定8は
現行の契約（`candidate-search-api.yaml`・`candidate-search-browser-interface.yaml`の該当バージョン）
どおり有効であり、この追記はその事実を変えない。詳細と判定根拠は`adr/0020`を参照する。
