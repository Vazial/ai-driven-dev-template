...
PR #65で `contracts/candidate-search.feature`、`contracts/candidate-search-api.yaml`、
ADR-0004がマージされ、候補探索の振る舞いとAPI形状は確定した。次の設計骨格合意では、幹事が
コンセプトを選び、候補カードと地図で比較し、再提案で前の応答を置き換える体験を、実装前に
確認可能にする必要がある。

この確認は実店舗、非公開の検索基点、provider応答、資格情報を必要としない。一方、レビュー用画面を
本番Djangoアプリまたはprovider clientとして扱うと、ADR-0002の公開リポジトリ／runtime境界と、
ADR-0004の非公開基点・地図通信の境界を誤って広げるおそれがある。
...
コンセプト数、検索範囲、APIフィールド、状態遷移を新たに決めない。receiverの設計は、少なくとも
TDR-CS-00〜08の通常・未認証・空・安全なエラー・rate-limit状態、コンセプト選択、カード／マーカーの
相互強調、前の応答を置き換える再提案を、合成状態としてレビュー可能にしなければならない。
...
- **静的なスクリーンショットだけでレビューする**: 状態、コンセプト選択、カードとマーカーの相互作用、
  再提案の置換を確認できないため不採用。
