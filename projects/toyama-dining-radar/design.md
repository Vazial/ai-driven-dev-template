...
runtime検索条件
  ...
  -> 順位付け後の上位5件を切り口の候補として選ぶ（ADR-0015。表示上限であり、取得・順位付け
     対象の上限ではない）
...
「もう一度探す」は、別の切り口を選ぶ操作とは異なる。独自の順位付け基準を持つ切り口（コンセプト）ではなく、現在表示中の切り口をそのまま使って新しい提案を依頼するだけの操作であり、新しい候補が現れるかどうかは、既表示候補の二次的な順位低下だけに依存する（乱数や決定性の緩和は行わない。ADR-0016）。2度目の実データレビューで、切り口の一つ（ジャンルを横断して比較する切り口）が実データ上ほぼ常に初期切り口と同一の候補・順序へ縮退することが判明し、その切り口を廃止して「もう一度探す」に置き換えた。
