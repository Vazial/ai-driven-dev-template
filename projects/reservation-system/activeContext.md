...
1. スライスRSV-KのPR作成 → CI → 人間マージ
2. 規程改善バッチ#2の候補(人間と協議済み・ADR起票待ち):
   - L4応答検証のOpenAPIスキーマ自動照合(requiredフィールド検証の機械化。RSV-K差し戻しの再発防止)
   - seam仕様の「テストインフラ契約」ファイル化(FR-006、同原因2回)
   - 監査注記の追跡ルール(FR-007。次スライス契約起草時のチェック項目化)
   - 監査レポート雛形に「承認者向けサマリ」節を必須化(FR-008)
3. 中期: B層「ドメインモデルパック実体」を本プロジェクトの実証済み部品から抽出(ビルド骨格・ArchUnit・seamパターン・CI)
