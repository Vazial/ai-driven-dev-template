...
垂直スライス4本（予約の作成RSV-C / キャンセルRSV-K / 空き枠確認RSV-A / 予約ルール確認RSV-R）が全段緑・マージ済み。以降は規程整備が続いており、いまはADR-0014（検証インフラの所有明示）を進めている（ブランチ meta/adr-0014-verification-infra）。

## 次にやること

1. ADR-0014のPR → CI（L4にログ清潔チェックが初めて載る）→ 人間マージ
2. **③ B層「ドメインモデルパック実体」の抽出**（このセッションで）。4スライスで実証済みの部品を切り出す: ビルド骨格（Gradle+受け入れテスト隔離ソースセット）・ArchUnit 4ルール・seam＋テストインフラ契約・DSL/step層分離・CI L0〜L4・checkstyle。未検証の判断（スナップショットの扱い＝設定変更スライスが無く未検証／軽量版パック＝ADR-0005で例示のみ）は`未検証`と明記して除外。パック実体の初作成なので構成自体がADR案件
3. **① 承認アーティファクトの一般ルール化**（govlintが cause_key `approval-artifact-readability-convention-missing` の3回目 FR-002/003/008 を報告中）: 雛形を足し続けず「人間が承認する成果物は先頭に平易な要約を置く」を一般則へ昇格するADR
4. **govlintに単体テストを付ける**（ADR-0014の宿題。現状テスト無し＝手作業検証の残骸。developerがテスト付きで持ち直す）
5. **クロスセッション経路の実地検証**: ルーティン化したスライスをフレッシュセッションに「HANDOFF読んで進めて」で拾わせる（成果物抽出③とは結合させない）
