...
## 主要な流れ(空き枠確認) — スライスRSV-A「空き枠を確認できる」(承認済み 2026-07-16)

> この節は契約ドラフト(contracts/reservation-availability.feature、contracts/reservation-api.yamlのRSV-A追記)に
> 伴う設計骨格の記述(承認済み)。

```
GET /rooms/{roomId}/availability?date=...
  → adapter/api: リクエスト検証・詰め替え
  → application: 部屋を取得(存在しなければ404)
    → その会議室のその日の予約(cancelled_atがNULLのものだけ)を取得
    → 営業時間からそれらの予約時間帯を除いた残り(空いている時間帯)を計算する
    → そのうち最小予約時間(30分)以上の連続空きだけを残す(予約可能な空きのみ。adr/0006)
  → 200(空いている時間帯一覧。0件もありうる) or 404(部屋が存在しない)
```

設計判断の要点:

- **CQRSの読み側**(docs/workshop-summary-01-reservation.md「CQRSで旧スキルが活きる」)。この経路は書き込み用の
  `Reservation`集約を生成・保存しない。空き時間帯の計算は読み取り専用の手順であり、予約作成時の不変条件検証
  (TimeSlot生成時のバリデーション等の書き込み側ルール)を再実行する必要はない
- 時間帯の半開区間`[start, end)`(終了時刻を含まない)の意味論は書き込み側と同じものを用いる。整合のため、
  domainの`TimeSlot`値オブジェクト(比較・包含判定)を読み取り専用の計算に再利用してよい。ただし読み取り経路は
  `Reservation`集約や書き込み用リポジトリを経由しない
- 空き状況の基準にする営業時間は、会議室の**現在の設定**を使う(過去の予約が持つスナップショットではない)。
  空き枠確認は「今この設定で予約したら空いているか」を答えるものであり、過去の特定予約の条件を再現するもの
  ではないため
- 新しいテーブル・パッケージは不要: 既存のrooms・reservationsテーブルの読み取りのみで完結する。既存の
  `adapter/api`にエンドポイントが1つ、`application`に読み取り専用のユースケースが1つ増える想定(具体的な
  クラス設計は実装判断)
- 受け入れテスト用の新しいseamは不要(公開APIのGET一本と、既存のGiven seam`POST /test-support/rooms`・
  `POST /reservations`の組み合わせで全シナリオを構成できる見込み)

## 主要な流れ(予約ルール確認) — スライスRSV-R「会議室の予約ルールを確認できる」(承認済み 2026-07-17)

> この節は契約ドラフト(contracts/reservation-rules.feature、contracts/reservation-api.yamlのRSV-R追記)に
> 伴う設計骨格の記述(承認済み)。
...
GET /rooms/{roomId}/rules
  → adapter/api: リクエスト検証・詰め替え
  → application: 部屋を取得(存在しなければ404)
    → 部屋の現在の営業時間・定員(roomsテーブルの現在値)と、システム共通の最小予約時間
      (RSV-C-05/RSV-A-05のルールが参照する値と同一箇所)を組み立てて返す
  → 200(予約ルール) or 404(部屋が存在しない)
```

設計判断の要点:

- **CQRSの読み側**。RSV-Aの空き枠確認と同型の読み取り専用経路で、`Reservation`集約や書き込み用リポジトリを
  経由しない
- 最小予約時間は**ドメイン1箇所の値をそのまま読み取る**(値の複製を作らない)。ADR-0006が示した
  「最小予約時間ルールをドメインの1箇所に保つ」方針をこのスライスでも継続する
- 営業時間・定員の基準は会議室の**現在の設定**を使う(RSV-Aの空き枠確認と同じ考え方)。現時点では
  会議室の設定を変更する公開APIが存在しないため、この区別は今のところ実務上は顕在化しない
- 新しいテーブル・パッケージは不要: 既存のroomsテーブルの読み取りのみで完結する。既存の`adapter/api`に
  エンドポイントが1つ、`application`に読み取り専用のユースケースが1つ増える想定(具体的なクラス設計は
  実装判断)
- 受け入れテスト用の新しいseamは不要: 公開APIのGET一本と、既存のGiven seam`POST /test-support/rooms`の
  組み合わせで全シナリオを構成できる見込み
