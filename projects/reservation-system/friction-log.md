...
## FR-020: PRの種別によって人間が判断すべきことは全く違うのに、すべて同じ重さで出していた（過剰と過少が同時に出た）
...
pushed_to: [meta/adr/0041-pr-kinds-and-approval-need.md]
status: 対応済み
principles: [P-04, P-08]
```

- 事象: 人間から「PRの出し方は考えてほしい。**昇格ならユーザが合意してるからレビューいらないよね？
  とかの区別ができていない**」と指摘された。実例が両方向に出ていた——**過剰**: #55（昇格PR）に
  DoD充足の表・承認方式の選択・各スライスの論拠を書いたが、中身は既に人間が承認・マージした13コミットの
  積み上げで**新しい決定はゼロ**だった。**過少**: #60（同期PR）は形式上「main取り込みのみ」だが、実際には
  activeContext の衝突解決で「解決済み項目を『（解決済み）』と書いて残さず削除する」という編集判断を
  しており、それが本文の中ほどに埋もれていた
- 原因の仮説: 成果物の形が受け手の必要に合っていない — orchestrator が**PRの形（base/head）だけを見て
  一律のテンプレを適用**し、「このPRで人間は何を判断するのか」を問うていなかった。**形と判断の要否は
  一致しない**（衝突解決を含む同期がその反例）。読む必要のないものを読ませ、読むべきものを埋もれさせる
  という、注意の配分の誤りである。テンプレが「必須項目」を並べる構造だったことも、機械的に全部埋める
  誘因になっていた
- 押し込み先: meta/adr/0041 — PRを4種別（スライス／meta／昇格／同期）に分類し、判定基準を
  「**そのPRに人間がまだしていない決定が含まれるか**」に置く。PRテンプレ冒頭に種別の選択を追加し
  （ADR-0035 が承認方式の二択を載せたのと同じ、全PRが通る関所）、`meta/guardrails.md` §2 に条文化。
  **機械検証はどの種別でも省略しない**ことを明記（「承認事項なし」と「検証不要」の混同が最も避けたい誤読）
- 補足: 同じ「一律に出す」形の誤りが他の成果物にも無いか——activeContext・friction-log・監査レポートは
  受け手（人間・次のセッション・agent）が異なるが、いずれも書式が固定されている。本FRでは扱わないが、
  **受け手の必要に合わせて濃淡をつける**という観点はPRに限らない。同種の指摘が再発したらこのcause_keyを
  再利用すること

---

## FR-021: 契約の承認記録がドリフトし、さらに未承認の契約の上に実装が載っていた（FR-015と同一機構の2回目）

```yaml
id: FR-021
date: 2026-07-31
found_at: AI
slice: RSV-T（会議室の登録）の契約起草の検証中
agents: [orchestrator]
cause_category: 記録の更新が別PRを要求する
cause_key: record-update-needs-second-pr
pushed_to: [meta/adr/0043-contract-approval-record-convention.md]
status: 対応済み
principles: [P-01, P-04, P-08, P-10]
```

- 事象: RSV-T の契約ドラフトを検証する過程でリポジトリ全体の契約のステータス行を確認したところ、
  **`my-reservations.feature`（RFE-C）が PR #49 で人間に承認されたのに「承認待ち」のまま**だった。
  さらに履歴を追うと**より重い事実**が出た——**RFE-A・RFE-B は契約が一度も承認されていないのに実装が
  本番コードとして載っている**。RFE-B は契約と実装が同一コミット（`f1dac2a`、ラベルは断面②）で、
  ADR-0022 の「断面①（契約承認）→断面②（実装）」の順序を通っていない。RFE-A は「設計記録」の
  docsコミットで導入された。**フロントで契約承認点が実際に機能したのは RFE-C が初めて**だった
- 原因の仮説: 記録の更新が別PRを要求する — **FR-015と完全に同一の機構**。契約は承認前に起草するので
  「承認待ち」と書くしかなく、承認行為はPRのマージで、記録には2本目のPRが要るため飛ばされる。
  ADR-0035 はこれをADRについて直したが**契約には適用していなかった**。加えて、ADRのステータスは
  frontmatter で govlint が機械検証するのに対し、**契約のステータスは行コメントで検証対象外**だった
  ため、ドリフトしても誰も気づけなかった。reservation-system の5本に日付が入っているのは当時2本目の
  コミットを打っていたからであり、**規律で維持していたものが続かなくなった**という点でもFR-015の
  結論（規律の呼びかけでは直らない）の再確認である
- 押し込み先: meta/adr/0043 — 契約もADRと同じ承認方式の二択を起草時に選ぶ／govlint が契約の
  ステータス行を機械検証し（形式不正はERROR、承認待ちはREPORT）、REPORT文面に「**この契約の上に実装が
  載っていないかも確認すること**」を含める／RFE-C の記録漏れのみ是正し、**RFE-A・RFE-B は実際に未承認
  なので「承認待ち」のまま残して可視化し続ける**（遡って承認したことにしない）
- 補足: cause_key を FR-015 と同じ `record-update-needs-second-pr` に**意図して揃えた**。対象は
  ADRと契約で違うが機構は完全に同一であり、**同一キーの2回目としてgovlintが構造的欠陥のシグナルを
  出すこと自体に意味がある**（同じ形が他の成果物——design.md・ARCHITECTURE.md・監査レポート——にも
...
## FR-023: ADR採番の衝突が2回目。1回目は規程を守っても防げない競合だったが、今回は規程を守らなかった
...
- 事象: RSV-T の昇格に向けた試行マージで govlint が
  `id が scope 内で重複している（meta#0044）` を出した。Codex が main に
  `meta/adr/0044-codex-efficient-verification-and-browser-use.md` を追加しており、
  `project/reservation-system` 側の `meta/adr/0044-contract-first-pending-scenarios.md` と
  衝突していた。Codex 側が既に main に載っているため、こちら（未昇格）を **0045 に振り直した**。
  ADR本文・ルートactiveContext・ADR-0043の訂正注記・`build.gradle` のヘッダ注記・FR-014 の
  `pushed_to` の参照をすべて追随させた
- 原因の仮説: 既存の規程を確認せず採番した — **ADR-0026 は「メタADRの採番は、mainだけでなく
  オープン中のPRも確認してから番号を取る」と明記している**。ADR-0043 の起草時にはこの確認を実施
  していた（`gh pr list --state open` を実行）のに、**続く 0044 では飛ばした**。0044 は RSV-T の
  スライスブランチ上で「その場で」書き始めたため、ADR起草の定型手順（main の最新採番＋オープンPRの
  確認）を通らなかった。**1回目の衝突（0037、2026-07-30）はオープンPRが空の状態から Codex が後から
  作成・マージしたもので規程を守っても防げない競合だったが、今回は違う——規程は存在し、守らなかった**
- 押し込み先: **未定（人間判断）**。候補: (a) ADR起草を「新しいブランチを main から切る」ことと
  不可分にする（そうすれば採番確認が自然に手順に入る）、(b) govlint の採番衝突検出は既に機能して
  いるので、**検出は足りている**と割り切り運用で回す。実際、今回も govlint が昇格前に捕まえており
  **main を汚さずに済んでいる**——検出機構は正しく働いた。「採番前の確認」を強制する仕組みを足すコストと、
  「衝突しても昇格前に検出できる」現状のコストを比べる判断が要る
- 補足: ルート activeContext には「**FRの採番にADR-0026相当の規定がない**」が未対応で記録されているが、
  今回判明したのは逆で、**ADRには規定があるのに守られなかった**という別種の問題である。規定の有無と
  遵守の有無は別の論点であり、混同しないよう本FRで分けて記録する
