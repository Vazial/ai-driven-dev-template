# activeContext.md — Connpass Session Radar

> P-11: このファイルは常に現在だけを映す。履歴はgitとADRが持つ。

## 現在

connpassの条件に合うイベントを毎朝1通届ける、画面・永続状態を持たないパイプラインである。興味条件は
リポジトリ内のYAML、通知先はDiscord Incoming Webhook、起点はGitHub Actionsのスケジュール実行、対象期間は
`windowDays`である（ADR-0001）。外部I/Oはconnpass API v2の取得と通知だけで、NotifierPortは将来の
通知先の差し替えを許す。

`daily-digest-test-support.yaml` v0.3は承認済みである。ADR-0003によりADR-0002のSlack案を実運用前に
取り止め、既定NotifierをDiscord Incoming Webhookへ置き換えた。実装も通常はEmbed、6,000文字超過時は
完全一覧を同じメッセージへ添付し、mentionを無効化して`wait=true`で送るDiscordアダプターへ置換した。
NotifierPortとCSR-D-01〜10の振る舞いは変わらない。

CSR-D-01〜10実装とL4 translationは完了している。すべてのCSR-Dシナリオから`@pending-implementation`を
外している。既存のreviewer監査対象だったsteps/DSLと受け入れbridgeは変更していない。

2026-08-22のreviewで見つかった3つの穴を実装側で塞いだ（契約・シナリオは変更していない）。

- 興味条件YAMLの未知フィールドと型違いを例外にする。`keywordAny:`のような誤記は従来そのまま素通りし、
  絞り込み条件ゼロのリクエスト＝connpass全件配信に化けていた。契約の`additionalProperties: false`へ
  実装を寄せた
- `CONNPASS_API_KEY`欠落と条件ファイルの読めない朝も、失敗通知を1通配信する。従来はパイプラインに
  入る前にthrowして無通知だった（product-brief §6・CSR-D-04）
- 失敗の理由を注入されたsinkへ報告し、スケジュール実行ではworkflowログへ出す。受信者向けの要約は
  従来どおり内部事情を含まない。受け入れ実行では沈黙する（canaryをログへ出さない）

続けて、届く一覧の書式を読みやすさ側へ組み替えた（人間が2026-08-22に提案を採用）。開催日ごとの
見出し、`Asia/Tokyo`の壁時計（`8/22(土) 19:00`。ISO-8601の生値をやめた）、タイトルをMarkdownリンクに
してURL行を廃し、時刻・場所・主催・残席を1行にまとめた。Embedのtitleに期間と件数を載せる。
`formatDigest`は`{ title, body }`を返すようになり、`NotifierPort`の第2引数もその形になった——
`NotifierPort`の契約（`DailyDigest`と`NotifierResult`）は変えていない。CSR-D-01〜10の観測は
`DailyDigest`を見るため、L4は無改変で緑のままである。同じ5件で本文は728文字から608文字へ短くなった。

興味条件を実運用の内容へ入れ替えた（2026-08-23の人間の聞き取り）。AI/LLM・アーキテクチャ設計・
クラウド・コンテナ/IaC・言語の5プロファイル（いずれも`prefectures: [online]`、7日）と、話題を絞らない
地元プロファイル（`prefectures: [toyama]`、14日）の6本である。地域を書く判断は`adr/0004`が記録した——
公開リポジトリなので関心領域と生活圏が公開される。あわせて`accepted`を`attendeeCount`として
NormalizedEventへ足し（契約 v0.2.0-draft）、メタ行に`148人`として出す。「申込者が多いもの」を知りたい
という要求に対し、connpass APIには参加者数で絞る・並べる手段が無いため、値を示して読み手に委ねる。
行頭のコメントがパーサを落としていた欠陥も直した（プロファイルに見出しを付けられなかった）。

**2026-08-23、初めて実データが届いた。その結果を受けて窓を二軸へ組み替えた（`adr/0005`）。**
初日は78件・約9,900文字で、Discord Embedの6,000文字上限を超えて添付テキストへ退避した——Markdownとして
描画されず、`product-brief` §1の「読みやすさ」が初日から成立しなかった。加えて7日窓は同じイベントを
7回くり返す（翌朝の78件中73件が前日と同じ）。

- **新着軸**（`publishedWithinDays`）: 昨日公開されたものを、開催日と無関係に拾う。connpassの
  `publish_ymd`を使う。三か月先のカンファレンスが今日告知されても今日の一覧に載るため、リード軸だけ
  では取り逃す「開催直前に告知されたもの」を拾える
- **リード軸**（`startsInDays` + `windowDays`）: ちょうど7日後に開催されるものだけ
- 1プロファイルで両方は使えない。`publish_ymd`と`ymd`を同一リクエストへ渡したときの結合の意味が
  一次情報に無いため、機械的に禁じている（P-08）
- **`catchPhrase`（主催者の1行、connpassの`catch`）を追加**。「どれがいいか判断できない」への答えで
  あり、**生成AIによる選定・要約は入れていない**（`product-brief` §8の除外を維持。要求の実体は
  判断材料の不足だった）
- 語から`設計`を外した。実データで「研修設計」「問いの設計」を拾っていた

見込みは約20件・約3,300文字でEmbedに収まる。**推定であり、次の実行が実測を出す。**

検証は自分で実行した: L0 green、Node tests 34/34（通常・`TZ=UTC`）、syntax green、L4 CSR-D-01〜10 green
（steps/DSLは無改変）。

## 保留・外部事実

- APIキーはローカル専用設定から値を表示せずに使用し、2026-08-22に実connpass API v2への取得を確認した。
  値はGit・ログ・成果物へ保存していない。
- **未着手の宿題（2026-08-22のreview）**: (1) 実測したAPI事実（`ymd`のOR結合、v2がイベントに
  `prefecture`を返さないこと）が`connpass-api-v2-facts.md`へ昇格しておらず、上書き運用の本ファイルに
  しかない。(2) ルート`activeContext.md`のプロジェクト行が「実装は一行も無い／通知先未定」のままで、
  昇格PRで直す必要がある。あわせて`adr/0004`決定3により、宿題(b)（契約の例示「富山県内の勉強会」）を
  昇格PRで閉じる。(3) 定員なし＋補欠ありを満席と示すか（CSR-D-09の文言との差）と、
  `prefectures`が`['online']`単独のときだけオンライン表示になる限界は、人間の判断で見送った。
- **未検証で、次の実行が答えを出すもの**: (a) 6プロファイルで実際に何件届くか——広すぎると毎朝
  6,000文字を超えて添付1個になり、`product-brief` §1の読みやすさが壊れる。調整のレバーは
  `windowDays`とキーワードの粒度であり、どちらも1行のcommitで効く。**測ってから決める**と人間が判断した
  （2026-08-23。推定で設計を組み替えないためにP-05を選んだ）。**測るときに見るのは件数だけではない**——
  現在の`[今日, 今日+windowDays)`という窓は、**同じイベントを`windowDays`回くり返し載せる**ので、
  毎朝の一覧の大半は前日の再掲である。実際に読んで、この重複がどれだけ negative に効くかを見る。
- **次に再燃しそうな論点**: (1) **定期開催シリーズの反復**——「0からのClaudeCodeによるAIエージェント
  入門セミナー」が別IDで週3回載っていた。**二軸にしても消えない**（毎回が新規公開のため）。除外語が
  要る場面だが、人間は2026-08-23に「まだ入れない」と判断した（`product-brief` §8の保留を維持）。
  (2) **HTMLページ化**——Discordは添付の`.md`も`.html`も描画しないため、文書として読みたいなら
  ホストが要る。これは §5「画面を持たない」を覆す判断であり、SkillMap構想と同じ置き場を必要とする。
  まず二軸でEmbedに戻った状態を見てから判断する。
- ~~**重複が問題なら採る案（2026-08-23に検討済み。まだ決めていない）**~~ **採用した（`adr/0005`）**:
  「新着軸」＋「リード軸」の二本立て。
  connpass APIには**開催日`ymd`とは別に公開日`publish_ymd`がある**ので、「昨日公開されたもの（開催日は
  問わない）」を拾える——これが**開催N日前より後に告知されたカンファレンスを取り逃す**という、リード固定
  だけにした場合の穴を塞ぐ。リード軸（ちょうどN日後に開催）は、告知時に見て忘れたものを参加判断の頃に
  思い出させる。各イベントは最大2回になり、状態は持たないまま（どちらもカレンダーとAPIパラメータだけで
  決まる）。**採用には再承認が要る**——`product-brief` §1が「新着の速報性は主目的ではない」「『今日の
  一覧』型であり、新着差分ではない」と明示的に決めており、`InterestProfile`にも公開日・リード日の概念が
  無いため契約変更を伴う。(b) `keyword`(AND)と
  `keyword_or`(OR)を同一リクエストへ同時に渡したときの結合の意味は一次情報に無い。現在の条件は
  **プロファイルごとにORグループ1つだけ**を使い、この空白を踏まない形にしてある。「AWSの深い話だけ」の
  ように話題×深さで絞るには結合の実測が要る。
- `ymd`の複数値はOR結合であることを実測した。2026-08-22の114件と2026-08-28の60件は重複0件で、
  反復指定とカンマ指定はいずれも和集合と同じ174件だった。現在の反復指定実装を維持する。
- commit済み条件（AWS・オンライン・7日間）で実データ11件を取得し、1,672文字のEmbed用一覧を生成した。
  API v2はイベントへ`prefecture`を返さないためオンライン表示を誤る事実を実測で発見し、オンライン限定
  profileから取得したイベントをオンラインとして正規化する修正と回帰テストを追加した。全11件がオンライン
  表示になり、重複した「オンライン / オンライン」は0件である。
- 通常テキストチャンネルのDiscord Incoming Webhookへ同じ実データ11件を1通のEmbedとして配信し、
  2026-08-22に人間が受信内容を「とりあえずOK」と確認した。mentionは発生していない。フォーラムチャンネルの
  Webhookは`thread_name`または`thread_id`が必須で現在の契約対象外であるため、通常テキストチャンネルを使う。
- プロジェクト専用CIはL1→L4を直列実行する。scheduled workflowは毎日08:00
  `Asia/Tokyo`に設定し、手動実行も許す。scheduleはGitHubの仕様上、mainへの昇格後に既定ブランチの
  最新版として動く。ローカル経路で両実プロバイダへの接続は確認済みだが、GitHub Secretsへの
  `CONNPASS_API_KEY`・`DISCORD_WEBHOOK_URL`登録状態は未確認である。
- 実通信を含む人間確認まで完了した。PR #120は`project/connpass-session-radar`へマージ済みである。
  次はreviewスライスを同ブランチへ戻し、そのうえでmainへの昇格PRを出す。
