...
> 承認済みのADR-0001を投影した、設計の現在の地図。commit・review済みの実装は未確定であり、
> 別worktreeの未commitなdeveloper draftはこの承認済み設計へ投影しない。ここに新しい決定はない。
...
興味条件 YAML → 取得 → 絞り込み・算出 → 整形 → 通知
                   ↑                              ↓
             connpass API                    LINE Messaging API
```
...
- 通知: `NotifierPort`の背後で、既定のLINE Messaging APIへ配信する。将来の配信先はこの境界で置換できる。
