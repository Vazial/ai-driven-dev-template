...
> 承認済みのADR-0001とADR-0002を投影した、設計の現在の地図。ここに新しい決定はない。
...
興味条件 YAML → 取得 → 絞り込み・算出 → 整形 → 通知
                   ↑                              ↓
             connpass API                    Slack Incoming Webhook
```
...
- 通知: `NotifierPort`の背後で、既定のSlack Incoming Webhookへ配信する。将来の配信先はこの境界で置換できる。
