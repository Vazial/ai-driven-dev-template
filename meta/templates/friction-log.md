...
> **cause_key** は「どの具体的な欠落か」を示すkebab-caseのキー。**新しいFRを書く前に既存のcause_keyを見て、同じ原因なら同じキーを再利用すること**。既存を見る範囲は**全ての台帳**である（`projects/<p>/friction-log.md` と `meta/friction-log.md`）。
> 同一キーの2回目以降は `meta/tools/govlint.py` が構造的欠陥のシグナルとして報告する（判断は人間。lintは失敗させない）。報告は**台帳ごと**と**台帳を跨いだ横断**の2本立てである（meta/adr/0058）。
>
> **この雛形をどこに置くか**: プロジェクトの実装・契約・設計で起きた摩擦は `projects/<p>/friction-log.md`、テンプレート本体（`meta/` 配下の規程・雛形・道具と、それに沿った運用）で起きた摩擦は `meta/friction-log.md`（既に存在する）。
