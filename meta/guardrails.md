...
- `main` はGitHub ruleset `protect main`（2026-07-27時点で有効）で保護されている。`develop` はまだ存在せず、保護も未設定である。ADR-0028の承認・マージ後に、人間が `develop` を作成し、同じ保護（`pull_request`：PR経由のみ・直push不可／`non_fast_forward`：force push禁止／`deletion`：削除禁止／`required_status_checks`：`L0: 統治文書の整合(govlint)` を必須）をGitHub rulesetで設定する。設定はgit管理外のため、状態が変わった時は本行も更新する。
