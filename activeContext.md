...
- **meta/adr/0042（提案中・人間の選択待ち）**: **デプロイ（本番配備）の論点整理**。architectが選択肢とトレードオフを並べたもので、**方式は確定していない**（ADR-0035 方式(ii)＝記録のみ）。最大の論点は「**現在の『越境なし』は Vite dev server proxy に依存しており、本番では成立しない**」こと——`vite build` の成果物に proxy は含まれず、フロントの実fetchは `baseURL` を持たない固定相対パスで、これは同一オリジン前提の設計判断。別オリジン＋CORSを選ぶと `reservation-frontend/adr/0009 決定2` が明示的に不採用にした baseURL 設計が必要になり、バックエンドへのCORS追加＝**越境**（meta/adr/0023）が発生する。**人間が最初に答えるべきは論点5（どこに出すか）と論点6（段階）**で、他の論点はこれに従属する。
