...
| skill-map（参加してきた勉強会から技術の地図を作る） | **Claude** | **保留。着手しない**（2026-08-23）。成果物は `product-brief.md` 1本で、コードも契約もADRも無い。ブリーフは承認されたが、**その直後に規約を読んで、いまのAPIキーでは作れないと分かった**——API利用規約 第9条第1項が「申告した利用目的以外に利用者情報を使わない」と定めており、**参加履歴の取得はいまのキーが許された範囲に含まれていない**。**個人申請ではキーを1本しか持てない**ため用途ごとの申請でも逃げられない。**再開の条件は1つだけ**——connpassへ問い合わせて利用目的に参加履歴の取得を追加できること。**断られたら成立しない**（スクレイピングは規約で禁止され、API以外に取得手段が無い）。規約の読み自体は残っており、再配布に明文が無いこと・イベント名の知的所有権がconnpassに帰属すること（サイト利用規約 第4条）と、公開する場合の出口の見立てはブリーフ §4 にある。`project/skill-map` ブランチと保護rulesetは作成済み、シナリオIDプレフィックスは未予約 | `projects/skill-map/activeContext.md` |
