...
| connpass-session-radar（connpassの毎朝ダイジェスト通知） | **Claude**（2026-08-16にCodexの先行準備を引き継ぎ） | **CSR-D-01〜10を実装済み・L0〜L4全緑**。毎朝1回のパイプライン（取得→絞り込み→算出→整形→配信）がGitHub Actionsのスケジュール実行で動き、Discord Incoming Webhookへ1通届く（通知先はLINE→Slack→**Discord**と2度変わった。`adr/0001`決定2→`0002`→`0003`）。**2026-08-23に実運用が始まった**——secretsを登録し、08:00 JSTのスケジュールが自動で発火して配信に成功した。**初日の実データが設計を1つ書き換えた**: 78件・約9,900文字でDiscordのEmbed上限6,000を超え、添付テキストへ退避してMarkdownとして描画されなかった。窓を**二軸**（`adr/0005`）へ組み替えた——「昨日公開されたもの（開催日は問わない。connpassの`publish_ymd`）」と「7〜8日後に開催」の2本立てで、同じイベントを読む回数が7回から最大2回になる。**組み替え後の実測は9件・約1,800文字**（上限の3割）でEmbedとして描画された。新着軸は毎週ページが作られる定期開催に偏るため、大物を拾う経路はリード軸だけである。あわせて主催者の1行（`catch`）を載せた（**生成AIによる選定・要約は入れない**という`product-brief` §8の除外は維持）。**地域を公開リポジトリの興味条件に書く判断は `adr/0004`**——地名を使ってよいのは興味条件の値だけで、リポジトリ名・プロジェクト名・ブランチ名には使わない。**残る運用の穴**: `project/connpass-session-radar` に保護rulesetが無い（他4本にはある。ADR-0028が要求する作成が漏れている）。**昇格のたびにprojectブランチがGitHubの自動削除で消える**が、人間は「未完のままmainへ載せたことが本来の異常であり、消えるのは正しい挙動」と判断して対応不要とした（2026-08-23。必要になればmainから1コマンドで復元する） | `projects/connpass-session-radar/activeContext.md` |
| skill-map（参加してきた勉強会から技術の地図を作る） | **Claude** | **起こしたばかり**（2026-08-23）。成果物は `product-brief.md` 1本で、コードも契約もADRも無い。connpassの参加履歴・発表履歴（`users/{nickname}/attended_events/`）から、何をどれだけやってきたかを時系列と偏りの両面で見せる。用途は**振り返る／次に学ぶものを決める／人に見せる（名刺）**の3つで、3つ目を意識的に含めた判断が制約をほぼ決めている——**出力が人の目に触れる**ため、履歴を公開してよいかが最初の問いになる。`project/skill-map` ブランチと保護rulesetは作成済み。**シナリオIDプレフィックスは未予約**（実体が定まってから台帳に書く。CSRが予約先行で台帳と実体が食い違った反省）。**次にやることは順番に意味がある**——(1) connpass利用規約の再配布条項を読む（結論しだいで公開の形が変わる）、(2) 「attended」が参加登録か受付かを実測する（地図の意味が変わる）、(3) 分類を誰がやるかのアーキテクチャ選定ADR。**connpass-session-radarとは別プロジェクト**である（画面を持たない／状態を持たない／未来を見る、という同プロジェクトの中心的決定を3つとも反転する）。一次情報は `projects/connpass-session-radar/connpass-api-v2-facts.md` を参照し、再定義しない | `projects/skill-map/activeContext.md` |
