...
**進行中のメタ論点・宿題**:
- **CodexのホストCLI確認手順（人間合意 2026-07-30、ADR-0037と運用反映を同一PRでレビュー待ち）**: 通常サンドボックス内のPowerShellで`gh`を発見できなかっただけでホスト環境にも存在しないと誤判定した。利用不能報告の前にホスト側標準探索または既知の実体パスを読み取り専用で確認し、実体・認証・権限・networkを分離して報告する。`AGENTS.md`へのCodex固有手順とFR-017への記録は反映済みで、このPRのマージをADR承認とする。
