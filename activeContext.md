...
*人間の判断待ち*
- **friction収穫ループの `meta/adr/0049` が提案中**（2026-08-22）。溜まったセッションログ
  （149MB・28セッション）を走査し、書き損ねた friction を候補として拾い直す。`gh pr merge` の
  PostToolUse hook がトリガで、閾値未満なら何も出さない。**既知のFR（`FR-002`・`ADR-0040` の
  議論）を機械的に再発見できることを実測済み**。あわせて「`meta/adr/0046` の施錠は**ゲート**に
  かかるものであり、判定しない道具は施錠の外（`meta/loop/`）」という区別を明文化する。
  **効果測定も同PRに入れた**（`meta/loop/friction_trend.py`）。摩擦率のベースラインは
  **3.8%**（訂正17件／450件）＝26回に1回、人間が打ち直している。ただし**この流量では
  改善は測れない**——半減の検出に約7.5か月、3割減なら約23か月を要する。測れるのは2倍以上の
  悪化だけ（約3.6か月）。したがって指標は回帰検知に限る（ADR-0049 決定7）。
  **採番の注記**: 起草時点で並行していた未マージのメタADRは `0050`（designer）・`0051`
  （ask-humans）で、`0049` は空いていた（main の末尾は `0048`）。番号の衝突は無い。ただし
  `activeContext` は3本とも同じ節を触るため、mainへ入る順に解決が要る（ADR-0026の直列化）
