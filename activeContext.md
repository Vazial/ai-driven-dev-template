...
- 0056 アーキテクチャ選定の出力を「設計パック名1個」から**6つの設計軸ごとの決定**へ変える（ルールの置き場／状態の持ち方／外部との境界／層の厚み／UIの厚み／処理の駆動。各軸に**禁じる依存**を1つ以上添えL2の入力にする）。§4の5行の表は候補集合から**事例**へ降格し、消去法と他プロジェクトADR-0001の参照を禁じた。**B層は畳まない**——引力源はB層（一度も充填されず現在も空）ではなくA層の閉じた表だった
