...
- **meta層のfrictionを書く場所が無い**（2026-08-22に観測、**2026-08-23に実害が出た**）: `friction-log.md` は `projects/<p>/` にしか存在せず（`meta/templates/friction-log.md` は雛形）、テンプレート自身の運用のブレはどのプロジェクトにも属さない。**「実際に困るまで置き場を作らない」（P-05）という保留は、実害の観測で終わった**——cause_key `approval-artifact-readability-convention-missing`（承認成果物に平易な要約が無い）は `projects/reservation-system/friction-log.md` で3回出てADR-0016の一般則を生んだが、その後 2026-08-22「ADRが読みにくい」（→ADR-0053・PR #133）と 2026-08-23「PR本文が読みにくい」（→ADR-0057・PR #134）として**実質4回目・5回目が再発し、どちらも台帳に1件も残っていない**。加えてADR-0055の収穫hook（`meta/loop/harvest_friction.py`）は候補を出せるが、A層には書き込み先が無い（入口だけあって出口が無い）。**置き場・集計範囲・遡及の3論点を `meta/adr/0058` として提出中**（status: 提案中＝ADR-0035 方式(ii)。人間が3論点を選択したら同じPRで実施まで行う）
- **govlint の cause_key 再出現検出が friction-log ファイル単位**: プロジェクトを跨いだ再出現を検出できない。`orchestrator-as-substantive-source` は実質4回だが機械はファイルごとにしか数えない。**上記の読みにくさの件（C層3回＋A層2回）も同じ穴に落ちる形をしており、台帳を新設するだけでは5回目が5回目として見えない**。この穴は `meta/adr/0058` の論点2として一緒に判断にかけている
