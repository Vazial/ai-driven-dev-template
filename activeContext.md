...
**進行中のメタ論点・宿題**:
- **ADR-0035（起票予定・未着手）**: ADRの承認記録がドリフトする構造への対策。`提案中` で起草→マージが承認行為→承認を記録するには2本目のPRが要る、という手順が原因で、ADR-0031〜0034は「承認済み」でありながらfrontmatterが `提案中` のまま残った（ADR-0028だけは `approved_by: 本PRのマージをもって承認` 方式で1PRで閉じており正しく記録されていた）。対策方針は(1)PRテンプレでADRを含むPRに「マージ＝承認／記録のみ・承認は後日」の二択を強制、(2)govlintに `[REPORT] 提案中のまま滞留しているADRの棚卸し` を追加（意味判定なのでERROR化はしない。P-04）。friction-log起票も同梱する（AIの見逃しが関与）。
