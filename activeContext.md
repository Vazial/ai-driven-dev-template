...
| connpass-session-radar（connpassの毎朝ダイジェスト通知） | **Claude**（2026-08-16にCodexの先行準備を引き継ぎ） | **CSR-D-01〜10を実装済み・L0〜L4全緑**。毎朝1回のパイプライン（取得→絞り込み→算出→整形→配信）がGitHub Actionsのスケジュール実行で動き、Discord Incoming Webhookへ1通届く（通知先はLINE→Slack→**Discord**と2度変わった。`adr/0001`決定2→`0002`→`0003`）。**2026-08-23に実運用が始まった**——secretsを登録し、08:00 JSTのスケジュールが自動で発火して配信に成功した。**初日の実データが設計を1つ書き換えた**: 78件・約9,900文字でDiscordのEmbed上限6,000を超え、添付テキストへ退避してMarkdownとして描画されなかった。窓を**二軸**（`adr/0005`）へ組み替え済み——「昨日公開されたもの（開催日は問わない。connpassの`publish_ymd`）」と「ちょうど7日後に開催」の2本立てで、同じイベントを読む回数が7回から最大2回になる。あわせて主催者の1行（`catch`）を載せた（**生成AIによる選定・要約は入れない**という`product-brief` §8の除外は維持）。**地域を公開リポジトリの興味条件に書く判断は `adr/0004`**——地名を使ってよいのは興味条件の値だけで、リポジトリ名・プロジェクト名・ブランチ名には使わない。**残る運用の穴**: `project/connpass-session-radar` に保護rulesetが無い（他4本にはある。ADR-0028が要求する作成が漏れている）。**昇格のたびにprojectブランチがGitHubの自動削除で消える**が、人間は「未完のままmainへ載せたことが本来の異常であり、消えるのは正しい挙動」と判断して対応不要とした（2026-08-23。必要になればmainから1コマンドで復元する） | `projects/connpass-session-radar/activeContext.md` |
