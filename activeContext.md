...
- **`.claude/settings.json` の deny パターンに実在する穴**（ADR-0039帰結で記録、**未対応**）: prefix一致のため `Bash(git reset --hard*)` は `git reset -q --hard` を止められない（実測で通った）。`Bash(git branch -D*)` も変形やグローバルオプション付き呼び出しを想定していない。ガードレール変更は `meta/permissions.md` の「ゲート変更」相当のため別作業。
