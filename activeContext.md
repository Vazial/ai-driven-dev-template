...
| connpass-session-radar（connpassの毎朝ダイジェスト通知） | **Claude**（2026-08-16にCodexの先行準備を引き継ぎ） | **CSR-D-01〜10を実装済み・L0〜L4全緑**。毎朝1回のパイプライン（取得→絞り込み→算出→整形→配信）がGitHub Actionsのスケジュール実行で動き、Discord Incoming Webhookへ1通届く（通知先はLINE→Slack→**Discord**と2度変わった。`adr/0001`決定2→`0002`→`0003`）。興味条件は6プロファイル（AI/LLM・設計・クラウド・コンテナ/IaC・言語をonline 7日、地元をtoyama 14日）。**地域を公開リポジトリの興味条件に書く判断は `adr/0004`**——地名を使ってよいのは興味条件の値だけで、リポジトリ名・プロジェクト名・ブランチ名には使わない。**実データでの動作確認済み**（2026-08-22に実connpass API取得＋実Discord配信を人間が確認）。**残る運用の穴**: リポジトリのsecretsが空で `CONNPASS_API_KEY`・`DISCORD_WEBHOOK_URL` が未登録（人間の作業）／**`project/connpass-session-radar` に保護rulesetが無い**（他4本にはある。ADR-0028が要求する作成が漏れている）。次の判断は「毎朝の分量を1週間測ってから、窓の作り方を決める」（現在の窓は同じイベントを7回載せる。二軸案は同プロジェクトのactiveContextに記録済み） | `projects/connpass-session-radar/activeContext.md` |
